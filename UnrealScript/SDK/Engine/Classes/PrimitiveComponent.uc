...
enum ERBCollisionChannel
{
	...
	RBCC_FlexAsset
`if(`__TW_PHYSICS_)
	// matches RBCollisionChannelContainer(.uc) and ERBCollisionChannel(.h)
...
`if(`__TW_PHYSICS_)
	// matches ERBCollisionChannel(.uc) and FRBCollisionChannelContainer(.h)
...
`if(`__TW_)
// End of engine.  Add game specific vars here.
...
/**
 * Whether the primitive can be merged with other primitives to create a grouped "per object shadow". The primitive will only
 * be merged with other primitives that are sufficiently close together so that sufficient shadow resolution is retained.
 * NOTE: This is only supported for primitives whose LightPrimitiveInteraction is updated every frame, such as pawns and other dynamic actors.
 */
...
`if(`__TW_LIGHTING_MODIFICATIONS_)
/**
 * Changes whether the primitive supports per objecty shadows
 * @param ShouldAllow New value
 */
...
defaultproperties
{
	...
	VisibilityId=-1
	RBCollideWithChannels={(FlexAsset=TRUE)}

`if(`__TW_LIGHTING_MODIFICATIONS_)
	// Default shadow cull distance to 20m
