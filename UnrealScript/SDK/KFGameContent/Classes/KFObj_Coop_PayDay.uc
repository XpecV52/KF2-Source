...
simulated event ReplicatedEvent(name VarName)
{
	...
	{
		`log("[OBJECTIVE]" @ GetFuncName() @ "ObjectiveState =" @ ObjectiveState, bLogObjective);
...
function int GetPayDayBonusDosh(int DoshSpend)
{
	if ( ObjectiveState == OS_Active )
	{
		`log("[OBJECTIVE]" @ GetFuncName() @ "CASH BACK AMOUNT:" @ Round(float(DoshSpend) * 0.3f), bLogObjective);
		return Round(float(DoshSpend) * 0.3f);
	}

	`log("[OBJECTIVE]" @ GetFuncName() @ "NO CASH BACK!", bLogObjective);
