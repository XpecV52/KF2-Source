...
function LoginToGame()
{
	...
	{
		`log(`location@"Running Local login now!!! - Whoo E3");
		LoginLocal();
		return;
	}
	...
	{
		`log(`location@"WE are not logged in at start time");
		OSS.PlayerInterface.AddLoginStatusChangeDelegate(OnLoginStatusChange, 0);
...
{
	`log(`location@`showvar(bWasSuccessful)@`showvar(SessionTicket)@`showvar(PlayfabId));
...
{
	`log("OnPlayfabStoreReadComplete"@bSuccessful);
	class'GameEngine'.static.GetPlayfabInterface().ClearStoreDataReadCompleteDelegate( OnPlayfabStoreReadComplete );
...
{
	`log("OnPlayfabInventoryReadComplete"@bSuccessful);
	class'GameEngine'.static.GetPlayfabInterface().ClearInventoryReadCompleteDelegate( OnPlayfabInventoryReadComplete );
...
{
	`log("store data read"@bSuccessful);
...
function OnLoginStatusChange(ELoginStatus NewStatus,UniqueNetId NewId)
{
	...
	{
		`log(`location@"- Success -"@`showvar(NewStatus)@`showvar(eHasPlayfabUser));
		eHasPILogin = ECS_CompleteSuccess;
		
		if(eHasPlayfabUser != ECS_CompleteSuccess)
		{
			eHasPlayfabUser = ECS_AsyncWaiting;
			`log(`location@"PlayerInterface login success, assuming PSN login info set for playfab login"@OSS.PlayerInterface.GetPlayerNickname(0));
	...
	{
		`log(`location@"!!FAILED!!"@`showvar(NewStatus));
...
function OnLoginFailed()
{
	...
	`log(`location@`showvar(LoginAttemptNum)@`showvar(LoginAttemptedLast));
	...
	if(Manager == none)
	{
		...
		`log(`location@"There is no manager associated with this movie, how is that possible?");
		ScriptTrace();
	}
	else if(Manager.CurrentPopup != none)
	{
		`log(`location@"There is already a movie playing at this time:"$`showvar(Manager.CurrentPopup));
		ScriptTrace();
	}
...
function OnLoginToGameComplete()
{
	...
	`log("!!!!!"@`location@"We have 'Finished' Logging in, proceed !!!!!");
	`log(`showvar(eHasInternet)@`showvar(eHasPILogin)@`showvar(eSaveDataLoaded)@`showvar(eHasTSSUpdates)@`showvar(eHasPlayfabUser));
...
{
	LoginAttemptNum=0
	LoginAttemptedLast=0
	eHasInternet=ECS_UnTested
	eSaveDataLoaded=ECS_UnTested
	eHasTSSUpdates=ECS_UnTested
	eHasPlayfabUser=ECS_UnTested
	eHasPILogin=ECS_UnTested
	AutoLoginState=ECS_UnTested
}