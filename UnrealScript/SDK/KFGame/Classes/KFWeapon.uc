...
function DropFrom(vector StartLocation, vector StartVelocity)
{
	...
	if( Instigator != None && Instigator.InvManager != None )
	{
		...
		{
			`SafeDialogManager.PlayDropWeaponDialog( KFPawn(Instigator) );
		}
	}
...
simulated function ZoomIn(bool bAnimateTransition, float ZoomTimeToGo)
{
	...
	`SafeDialogManager.PlayIronsightsDialog( KFPawn(Instigator) );
}
...
simulated function bool ShouldForceSingleFireSound()
{
	...
	if ( `IsInZedTime(self) && SingleFireMode != FIREMODE_None )
	{
		return true;
	}
...
function int AddAmmo(int Amount)
{
	...
	`log(self@"MaxSpareAmmo[0] =" @ MaxSpareAmmo[0], bLogAmmo);
...
reliable client function ClientForceAmmoUpdate(byte NewAmmoCount, int NewSpareAmmoCount)
{
	...
	{
		// detect sync errors
		`log(self@"ClientForceAmmoUpdate ServerAmmo Primary:"$NewAmmoCount@"ClientAmmo Primary:"$AmmoCount[0]@"ServerSpareAmmo Primary:"$NewSpareAmmoCount@"ClientSpareAmmo Primary:"$SpareAmmoCount[0], bDebug);
		if ( NewAmmoCount != AmmoCount[0] )
		{
			`log(self@"ClientForceAmmoUpdate Primary Ammo mismatch Server:"$NewAmmoCount@"Client:"$AmmoCount[0]);
		}
		if ( NewSpareAmmoCount != SpareAmmoCount[0] )
		{
			`log(self@"ClientForceAmmoUpdate Primary SpareAmmo mismatch Server:"$NewSpareAmmoCount@"Client:"$SpareAmmoCount[0]);
		}
...
reliable client function ClientForceSecondaryAmmoUpdate(byte NewSecondaryAmmoCount)
{
	...
	{
		// detect sync errors
		`log(self@"ClientForceAmmoUpdate ServerAmmo Secondary:"$NewSecondaryAmmoCount@"ClientAmmo Secondary:"$AmmoCount[1], bDebug);
		if ( NewSecondaryAmmoCount != AmmoCount[1] )
		{
			`log(self@"ClientForceAmmoUpdate Secondary Ammo mismatch Server:"$NewSecondaryAmmoCount@"Client:"$AmmoCount[1]);
		}
...
simulated state GrenadeFiring extends WeaponSingleFiring
{
    ...
    simulated event BeginState( Name PreviousStateName )
	{
		...
		if ( Role == ROLE_Authority && KFInventoryManager(InvManager) != none )
        {
            ...
            // __TW_ANALYTICS_
            `RecordGrenadeThrown(Instigator.Controller, KFInventoryManager(InvManager).GrenadeCount);
        }
...
simulated state Reloading
{
	...
	simulated function BeginState(name PreviousStateName)
	{
		...
		`RecordPlayerReload(self, Instigator.Controller);
		`SafeDialogManager.PlayReloadDialog( KFPawn(Instigator) );
	}
...
`endif
		...
		`SafeDialogManager.PlayAmmoDialog( KFPawn(Instigator), float(SpareAmmoCount[0]) / float(MaxSpareAmmo[0]) );
	}
...
defaultproperties
{
	...
	// Zooming/Position
	PlayerViewOffset=(X=1.0,Y=9,Z=-3)
	...
	// ALTFIRE_FIREMODE
	FireModeIconPaths[ALTFIRE_FIREMODE]=none
	...
	// GRENADE_FIREMODE
	FiringStatesArray(GRENADE_FIREMODE)=GrenadeFiring
	WeaponFireTypes(GRENADE_FIREMODE)=EWFT_Projectile
	FireInterval(GRENADE_FIREMODE)=0.6
	...
	// Effects
	WeaponFireSnd(DEFAULT_FIREMODE)=()
    ...
    TargetFrictionDistanceMin=320.0f
    TargetFrictionDistancePeak=200.0f
    TargetFrictionDistanceMax=4000.0f
    TargetFrictionDistanceCurve=(Points=((InVal=0,OutVal=0.0),(InVal=320.0,OutVal=0.0),(InVal=2000.0,OutVal=1.0),(InVal=4000.0,OutVal=0.3),(InVal=9000.0,OutVal=0.0)))
    TargetFrictionMultiplierRange=(X=0.3f,Y=0.4f)
    TargetFrictionPeakRadiusScale=1.f
    TargetFrictionPeakHeightScale=0.2f
    TargetFrictionOffset=(Z=32.0f)
    TargetFrictionZoomedBoostValue=0.25f
    bTargetAdhesionEnabled=True
    TargetAdhesionDistanceMax=2000.0f
    TargetAdhesionAimDistY=128.0f
    TargetAdhesionAimDistZ=96.0f
    TargetAdhesionScaleRange=(X=0.3f,Y=2.85f)
    TargetAdhesionScaleAmountMin=0.0f
    TargetAdhesionTargetVelocityMin=0.0f
    TargetAdhesionPlayerVelocityMin=0.0f
