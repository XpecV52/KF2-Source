...
exec function GamepadSprintRelease()
{
	...
	else if ( bRun == 0 && `TimeSince(GamepadSprintPressTime) < GamepadButtonHoldTime )
	{
		ToggleCrouch();
	}
...
//	CamRot = CalcViewRotation;
	...
	if( FrictionTarget != None )
	{
			...
			)
		{
    		...
    		{
    			`log(GetFuncName()@"DistMultiplier: " $ DistMultiplier );
			}
...
{
	if ( `TimeSince(LastLogPerkTime) > 5.f )
	{
		if ( CurrentPerk != none )
		{
...
defaultproperties
{
    ...
    ViewAccel_TimeToHoldBeforeFastAcceleration=0.0625f
    ViewAccel_RampSpeed=0.0075f
    ViewAccel_MaxTurnSpeed=1.1f
	SensitivityScaleCurve=(Points=((InVal=0.f,OutVal=0.f),(InVal=0.1f, OutVal=0.07f),(InVal=0.25f, OutVal=0.2f),(InVal=0.5f, OutVal=0.5f),(InVal=0.75f, OutVal=1.25f),(InVal=1.0f, OutVal=1.8f))) // Max OutVal 2.0f without accelleration on
   	//HorizontalSensitivityScale=1.0f
	//VerticalSensitivityScale=0.9f//0.8f without accelleration on
	AutoTargetRotationRate=35
	AutoTargetAngleCurve=(Points=((InVal=0.f,OutVal=0.80f),(InVal=2500.0f, OutVal=0.98f)))
