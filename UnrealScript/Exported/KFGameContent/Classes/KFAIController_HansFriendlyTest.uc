...
function bool ShouldSprint()
{
	...
	if( Enemy != none && MyHansPawn != none && !MyHansPawn.bIsHeadless && !MyHansPawn.bEmpPanicked )
	{
        ...
        if( LastEnemySightedTime == 0 && (LastEnemyVisionCheckTime == 0 ||
            ((WorldInfo.TimeSeconds - LastEnemyVisionCheckTime) > EnemyVisionCheckInterval)) )
        {
		...
		{
            // Hack, since Hans doesn't get SeePlayer calls on zeds, check here to see if we can see our enemy
            if( LastEnemyVisionCheckTime == 0 || ((WorldInfo.TimeSeconds - LastEnemyVisionCheckTime) > EnemyVisionCheckInterval) )
            {
                LastEnemyVisionCheckTime = WorldInfo.TimeSeconds;
                if( CanSee(Enemy) )
                {
...
defaultproperties
{
   EnemyVisionCheckInterval=0.100000
   MaxGunAttackLength=30.000000
   MinShootingRange=100.000000
   ShootingCooldown=3.000000
   bAllowScriptTeamCheck=True
   Name="Default__KFAIController_HansFriendlyTest"
   ObjectArchetype=KFAIController_Hans'KFGame.Default__KFAIController_Hans'
}
