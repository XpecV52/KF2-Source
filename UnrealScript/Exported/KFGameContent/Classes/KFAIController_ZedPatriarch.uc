...
event Possess( Pawn inPawn, bool bVehicleTransition )
{
	...
	{
		WarnInternal(GetFuncName()$"() attempting to possess "$inPawn$", but it's not a KFPawn_ZedPatriarchBase class! MyPatPawn variable will not be valid.");
	}
    ...
    if( LastRecentSeenEnemyListUpdateTime == 0 || (WorldInfo.TimeSeconds - LastRecentSeenEnemyListUpdateTime) > RecentSeenEnemyListUpdateInterval )
    {
    	...
    	for( i = RecentlySeenEnemyList.Length-1; i >= 0; i-- )
    	{
			...
			if( RecentlySeenEnemyList[i].TrackedEnemy == none
				...
				|| (WorldInfo.TimeSeconds - RecentlySeenEnemyList[i].LastTimeVisible) > 0.5f )
			{
...
function EvaluateAttacks( float DeltaTime )
{
	...
	if( !MyPatPawn.bIsCloaking
		...
		&& (WorldInfo.TimeSeconds - LastChargeAttackTime) > MyPatPawn.ChargeAttackCooldownTime )
	{
	...
	bCanFireMinigun = (!MyPatPawn.CanChargeAttack() || fRand() < 0.5f)
						...
						&& RecentlySeenEnemyList.Length > 0 
						&& (WorldInfo.TimeSeconds - LastSuccessfulAttackTime) > MyPatPawn.MinigunAttackCooldownTime;
	bCanFireMissile = (!MyPatPawn.CanChargeAttack() || fRand() < 0.5f)
						&& (!MyPatPawn.bIsCloaking || fRand() < 0.75f)
						&& MyPatPawn.CanMissileAttack() && RecentlySeenEnemyList.Length > 0
						&& (WorldInfo.TimeSeconds - LastMissileAttackTime) > MyPatPawn.MissileAttackCooldownTime;
	bCanFireMortar = !MyPatPawn.bIsCloaking
						&& MyPatPawn.CanMortarAttack()
						&& (WorldInfo.TimeSeconds - LastMortarAttackTime) > MyPatPawn.MortarAttackCooldownTime;
...
function NotifyKilled( Controller Killer, Controller Killed, pawn KilledPawn, class<DamageType> damageType )
{
	...
	{
		if( Role == ROLE_Authority && KFGameInfo(WorldInfo.Game) != none && KFGameInfo(WorldInfo.Game).DialogManager != none) KFGameInfo(WorldInfo.Game).DialogManager.PlayPattyKilledDialog( MyPatPawn, damageType );
	}
...
function StartPaternalInstinct()
{
	...
	// Play a dialog event
	if( Role == ROLE_Authority && KFGameInfo(WorldInfo.Game) != none && KFGameInfo(WorldInfo.Game).DialogManager != none) KFGameInfo(WorldInfo.Game).DialogManager.PlayPattyChildKilledDialog( MyPatPawn );
...
function NextBattlePhase()
{
	bRagedThisPhase = false;
 	...
 	// Play a dialog event
 	if( Role == ROLE_Authority && KFGameInfo(WorldInfo.Game) != none && KFGameInfo(WorldInfo.Game).DialogManager != none) KFGameInfo(WorldInfo.Game).DialogManager.PlayPattyBattlePhaseDialog( MyPatPawn, MyPatPawn.CurrentBattlePhase );
...
defaultproperties
{
   ...
   AggroFalloffWaitTime=1.000000
   AggroFalloffPerSecond=25.000000
   ...
   MaxFanFireRangeSQ=490000.000000
   ...
   RageTimeOut=16.000000
   HumanBumpDamage=10
   HumanBumpMomentum=8000
   FleeHealthThreshold=0.350000
