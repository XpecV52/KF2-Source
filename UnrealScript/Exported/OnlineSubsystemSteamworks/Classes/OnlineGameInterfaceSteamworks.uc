...
struct native ServerQueryToPlayerListResponseMapping
{
	...
	var int Query;
	/** The Steam callback object */
	var native pointer Response{FOnlineAsyncTaskSteamPlayerListRequest};
...
/** Represents a game server stored as a favorite on Steam */
...
function SetMatchmakingTypeMode(ESteamMatchmakingType InMatchmakingTypeMode)
{
	switch (InMatchmakingTypeMode)
	{
		...
		case SMT_LAN:
			...
			break;
		default:
			LogInternal("Steam server search type" @ InMatchmakingTypeMode @ "not supported.");
...
{
	if (GetPlayerListCompleteDelegates.Find(GetPlayerListCompleteDelegate) == INDEX_NONE)
	{
...
/**
 * Adds the search result at the specified index to the favorites
 *
 * @param Index the index of the search result to add to the favorites
 *
...
/**
 * Removes the search result at the specified index from the favorites
 *
 * @param Index the index of the search result to remove from the favorites
 *
 * @return true if successful, false if not
 */
...
{
	LogInternal("Ignored on Steamworks");  // ignored on Live, too.
	return false;
}
...
defaultproperties
{
   ServerBrowserSearchQuery=(PlayerListQuery=(Query=-1))
   InviteSearchQuery=(PlayerListQuery=(Query=-1))
   MatchmakingTypeMode=SMT_Internet
   Name="Default__OnlineGameInterfaceSteamworks"
