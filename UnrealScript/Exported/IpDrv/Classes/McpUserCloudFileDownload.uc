...
function EnumerateUserFiles(string UserId)
{
	...
	{
		LogInternal("("$Name$") McpUserCloudFileDownload::"$GetStateName()$":"$GetFuncName()@"User files already being enumerated for"
			$" UserId="$UserId);
	}
...
private function OnHTTPRequestEnumerateUserFilesComplete(HttpRequestInterface Request, HttpResponseInterface Response, bool bWasSuccessful)
{
	...
	if (EntryIdx != INDEX_NONE)
	{
			...
			Response.GetResponseCode() == 200)
		{
			...
			if (JsonString != "")
			{
				LogInternal("("$Name$") McpUserCloudFileDownload::"$GetStateName()$":"$GetFuncName()@""
		...
		{
			LogInternal("("$Name$") McpUserCloudFileDownload::"$GetStateName()$":"$GetFuncName()@"Failed to enumerate files for"
				$" UserId="$UserCloudFileRequests[EntryIdx].UserId
				$" URL="$Request.GetURL());
		}
...
function bool ReadUserFile(string UserId,string FileName)
{
		...
		Len(FileName) > 0)
	{
		...
		{
			LogInternal("("$Name$") McpUserCloudFileDownload::"$GetStateName()$":"$GetFuncName()@"File operation already in progress"
				$" UserId="$UserId
				$" FileName="$FileName);
		}
	}
	...
	{
		LogInternal("("$Name$") McpUserCloudFileDownload::"$GetStateName()$":"$GetFuncName()@"Invalid parameters"
			$" UserId="$UserId
			$" FileName="$FileName);
	}
	// failed to kick off the request, always call the completion delegate
...
private function OnHTTPRequestReadUserFileComplete(HttpRequestInterface Request, HttpResponseInterface Response, bool bWasSuccessful)
{
	...
	{
		LogInternal("("$Name$") McpUserCloudFileDownload::"$GetStateName()$":"$GetFuncName()@"Couldn't find entry index");
	}
	// call the completion delegate since the HTTP request completed
...
defaultproperties
{
   ...
   Name="Default__McpUserCloudFileDownload"
   ObjectArchetype=McpServiceBase'IpDrv.Default__McpServiceBase'
}
