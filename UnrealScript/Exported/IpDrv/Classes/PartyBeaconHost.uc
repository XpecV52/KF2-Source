...
function DumpReservations()
{
	...
	LogInternal("Debug info for Beacon: "$BeaconName,'DevBeacon');
	LogInternal("Session that reservations are for: "$OnlineSessionName,'DevBeacon');
	LogInternal("Number of teams: "$NumTeams,'DevBeacon');
	LogInternal("Number players per team: "$NumPlayersPerTeam,'DevBeacon');
	LogInternal("Number total reservations: "$NumReservations,'DevBeacon');
	LogInternal("Number consumed reservations: "$NumConsumedReservations,'DevBeacon');
	LogInternal("Number of party reservations: "$Reservations.Length,'DevBeacon');
	LogInternal("Reserved host team: "$ReservedHostTeamNum,'DevBeacon');
	...
	for (PartyIndex = 0; PartyIndex < Reservations.Length; PartyIndex++)
	{
		...
		LogInternal("  Party team: "$Reservations[PartyIndex].TeamNum,'DevBeacon');
		LogInternal("  Party size: "$Reservations[PartyIndex].PartyMembers.Length,'DevBeacon');
		...
		for (MemberIndex = 0; MemberIndex < Reservations[PartyIndex].PartyMembers.Length; MemberIndex++)
		{
			...
			LogInternal("  Party member: "$class'OnlineSubsystem'.static.UniqueNetIdToString(PlayerRes.NetId)
