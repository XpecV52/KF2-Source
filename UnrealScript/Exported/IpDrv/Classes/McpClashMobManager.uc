...
function QueryChallengeList()
{
	...
	if (Len(ErrorStr) > 0)
	{
		LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@ErrorStr);
	}
	if (!bPending)
	{
...
function GetChallengeFileList(string UniqueChallengeId, out array<McpClashMobChallengeFile> OutChallengeFiles)
{
	...
	{
		LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@"Couldn't find event entry for"
...
private function OnLoadCachedFileComplete(bool bWasSuccessful,string DlName)
{
	...
	if (FileIdx != INDEX_NONE)
	{
		...
		if (bWasSuccessful)
		{
			...
			if	(Len(FileHashChallenge) > 0)
			{
				...
				{
					LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@ "Found challenge file in cache with valid hash. DLName="$DlName @"in file cache. Not downloading.");
		...
		{
			LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@ "Did not find challenge file DLName="$DlName @"in file cache with valid hash. Starting download.");
	...
	{
		LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@ "Could not find DLName="$DlName @"for challenge");
	}
}
...
function OnDownloadMcpFileComplete(bool bWasSuccessful,string DlName)
{
	...
	if	(FileIdx != INDEX_NONE)
	{
		...
		{
			LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@ "Downloaded challenge file. DLName="$DlName @". Copying to file cache.");
...
function GetChallengeFileContents(string UniqueChallengeId, string DlName, out array<byte> OutFileContents)
{
	...
	if (ChallengeIdx != INDEX_NONE)
	{
		...
		if (FileIdx != INDEX_NONE)
		{
			...
			{
				LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@"No data loaded for file entry."
					$" UniqueChallengeId="$UniqueChallengeId
					$" DLName="$DlName);
			}
		}
		...
		{
			LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@"Couldn't find file entry."
				$" UniqueChallengeId="$UniqueChallengeId
				$" DLName="$DlName);
		}
	}
	...
	{
		LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@"Couldn't find event entry."
			$" UniqueChallengeId="$UniqueChallengeId);
	}
}
...
function ClearCachedChallengeFile(string UniqueChallengeId, string DlName)
{
	...
	if (ChallengeIdx != INDEX_NONE)
	{
		...
		if (FileIdx != INDEX_NONE)
		{
			...
			{
				LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@"Can't clear. File download pending."
					$" UniqueChallengeId="$UniqueChallengeId
					$" DLName="$DlName);
			}
		}
...
function DeleteCachedChallengeFile(string UniqueChallengeId, string DlName)
{
	...
	if (ChallengeIdx != INDEX_NONE)
	{
		...
		if (FileIdx != INDEX_NONE)
		{
			...
			{
				LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@"Can't delete. File download pending."
					$" UniqueChallengeId="$UniqueChallengeId
					$" DLName="$DlName);
			}
		}
...
function GetChallengeUserStatus(string UniqueChallengeId, string UniqueUserId, out McpClashMobChallengeUserStatus OutChallengeUserStatus)
{
	...
	else
	{
		...
		LogInternal("McpClashMobManager::"$GetStateName()$":"$GetFuncName()@"Couldn't find user status for"
			$" UniqueChallengeId="$UniqueChallengeId
			$" UniqueUserId="$UniqueUserId);
	}
}
...
defaultproperties
{
   ...
   Name="Default__McpClashMobManager"
   ObjectArchetype=McpClashMobBase'IpDrv.Default__McpClashMobBase'
}
