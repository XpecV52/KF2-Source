...
function DumpConnections()
{
	...
	for (ClientIdx=0; ClientIdx < ClientConnections.Length; ClientIdx++)
	{
		...
		LogInternal("Client connection entry: "$ClientIdx,'DevBeacon');
		LogInternal("	PlayerNetId: "$class'OnlineSubsystem'.static.UniqueNetIdToString(NetId),'DevBeacon');
		LogInternal("	NatType: "$ClientConnections[ClientIdx].NatType,'DevBeacon');
		LogInternal("	GoodHostRatio: "$ClientConnections[ClientIdx].GoodHostRatio,'DevBeacon');
		LogInternal("	bCanHostVs: "$ClientConnections[ClientIdx].bCanHostVs,'DevBeacon');		
		LogInternal("	MinutesSinceLastTest: "$ClientConnections[ClientIdx].MinutesSinceLastTest,'DevBeacon');		
		LogInternal("	BandwidthTest.CurrentState: "$ClientConnections[ClientIdx].BandwidthTest.CurrentState,'DevBeacon');
		LogInternal("	BandwidthTest.TestType: "$ClientConnections[ClientIdx].BandwidthTest.TestType,'DevBeacon');
		LogInternal("	Bandwidth History: "$ClientConnections[ClientIdx].BandwidthHistory.Length,'DevBeacon');
