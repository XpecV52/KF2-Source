...
event FinishedAsyncUncompression(bool bWasSuccessful, const out array<byte> UncompressedMessageContents, string MessageId)
{
	...
	{
		LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName());
	}
	...
	const out array<byte> MessageContents)
{
	...
	if(CreateMessageRequest != none)
	{
		...
		else
		{
			...
			{
				LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()@"Failed to process web request for URL(" $ Url $ ")");
			}
			LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()$ " URL is " $ Url);
		}
	}
}
...
function OnCreateMessageRequestComplete(HttpRequestInterface CreateMessageRequest, HttpResponseInterface HttpResponse, bool bWasSuccessful)
{
	...
	if (HttpResponse != none && CreateMessageRequest != none)
	{
		...
		if ( bWasSuccessful)
		{
			LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()@"Message created.");
		}
		else
		{
			LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()@" CreateMessage query did not return a message.");
		}
	}
...
function QueryMessages(String ToUniqueUserId)
{
	...
	if (QueryMessagesRequest != none)
	{
		...
		LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()@"URL: " $ URL);
		// Call WebRequest
		...
		bWasSuccessful = bWasSuccessful && ResponseCode == 200;
			if (bWasSuccessful)
			{
				...
				else
				{
					...
					LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()$ Error);
				}
			}
			else
			{
...
function GetMessageList(string ToUniqueUserId, out McpMessageList MessageList)
{
	...
	{
		LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()$ " Requester Id not found or MessageLists is empty. Using ToUniqueUserId: " $ ToUniqueUserId);
	}
}
...
private function OnQueryMessageContentsRequestComplete(HttpRequestInterface OriginalRequest, HttpResponseInterface HttpResponse, bool bWasSuccessful)
{
	...
	if (HttpResponse != none && OriginalRequest != none)
	{
		...
		if (bWasSuccessful && Len(MessageId) > 0 )
		{
			...
			LogInternal("MessageId:" $ MessageId $" Compressed Message Contents Length:" $ MessageContents.Length);
...
function CacheMessage( McpMessage Message)
{
	...
	if(MessageListIndex != INDEX_NONE)
	{
		...
		LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()$ " MessageId: " $ UserMessageList.Messages[AddAt].MessageId);
...
function bool CacheMessageContents(const out array<byte> MessageContents, String MessageId)
{
	...
	{
		LogInternal("("$Name$") McpMessageManager::"$GetStateName()$":"$GetFuncName()@" Either the MessageContents or MessageId were not Specified.");
	}
	return bWasSuccessful;
}
...
defaultproperties
{
   ...
   Name="Default__McpMessageManager"
   ObjectArchetype=McpMessageBase'IpDrv.Default__McpMessageBase'
}
