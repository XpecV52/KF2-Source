...
function bool ReadTitleFile(string FileToRead, optional EOnlineFileType FileType = OFT_Binary)
{
	...
	else if (TitleFiles[FileIndex].AsyncState != OERS_InProgress)
	{
		...
		LogInternal("("$Name$") OnlineTitleFileDownloadWeb::"$GetStateName()$":"$GetFuncName()@ "starting read for title file"
			@"url="$URL);
...
private function OnFileDownloadComplete(HttpRequestInterface Request, HttpResponseInterface Response, bool bDidSucceed)
{
	...
	if (bDidSucceed)
	{
		...
		LogInternal("("$Name$") OnlineTitleFileDownloadWeb::"$GetStateName()$":"$GetFuncName()@ ""
			@"FileIndex="$FileIndex
		...
		{
			LogInternal("("$Name$") OnlineTitleFileDownloadWeb::"$GetStateName()$":"$GetFuncName()@ "No entry found for"
				@"FileIndex="$FileIndex);
		}
	}
	else
	{
		LogInternal("("$Name$") OnlineTitleFileDownloadWeb::"$GetStateName()$":"$GetFuncName()@ "web request for file download failed");
	}
...
function bool RequestTitleFileList()
{
	...
	{
		LogInternal("("$Name$") OnlineTitleFileDownloadWeb::"$GetStateName()$":"$GetFuncName()@"HTTPRequest object missing");
	}
	return true;
}
...
function OnFileListReceived(HttpRequestInterface Request, HttpResponseInterface Response, bool bDidSucceed)
{
	...
	if (bDidSucceed)
	{
		...
		{
			LogInternal("("$Name$") OnlineTitleFileDownloadWeb::"$GetStateName()$":"$GetFuncName()@"Download of file list failed. Bad response."
				@"ResponseCode="$Response.GetResponseCode()
				@"URL="$Request.GetURL());
		}
	}
	...
	{
		LogInternal("("$Name$") OnlineTitleFileDownloadWeb::"$GetStateName()$":"$GetFuncName()@"Download of file list failed.");
	}
...
defaultproperties
{
   ...
   TimeOut=10.000000
   Name="Default__OnlineTitleFileDownloadWeb"
