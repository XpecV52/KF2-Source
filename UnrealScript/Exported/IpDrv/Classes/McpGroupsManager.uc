...
function CreateGroup(String UniqueUserId, String GroupName)
{
	...
	if(Len(UniqueUserId) > 0 && Len(GroupName) > 0)
	{
		...
		if(CreateGroupRequest != none)
		{
			...
			{
				LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()@"Failed to process web request for URL(" $ Url $ ")");
			}
			LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()$ " URL is " $ Url);
		}
		else
		{
...
function OnCreateGroupRequestComplete(HttpRequestInterface CreateGroupRequest, HttpResponseInterface HttpResponse, bool bWasSuccessful)
{
	...
	if (HttpResponse != none && CreateGroupRequest != none)
	{
		...
		{
			LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()@" CreateGroup query did not return a group.");
		}
	}
	...
	if (HttpResponse != none && OriginalRequest != none)
	{
		...
		if (bWasSuccessful)
		{
			...
			else
			{
				...
				LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()$ Error);
			}
		}
		else
		{
...
function GetGroupList(string UserId, out McpGroupList GroupList)
{
	...
	if( Len(UserId) > 0 )
	{
		...
		{
			LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()$ " Requester Id not found or GroupLists is empty. Using UserId: " $ UserId);
		}
	}
	else
	{
		LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()$ "UserId not specified");
	}
}
	...
	if( Len(UniqueUserId) > 0 && Len(GroupId) > 0)
	{
		...
		if(AddGroupMembersRequest != none)
		{
			...
			if(MemberIds.Length > 0)
			{
				...
				LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()@"URL(" $ Url $ ")");
			}
			else
			{
				LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()@" No MemberIds given.");
			}
		}
		else
		{
...
function CacheGroup(string RequesterId, McpGroup Group)
{
	...
	if(GroupListIndex != INDEX_NONE)
	{
		...
		LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()$ " GroupName: " $ UserGroupList.Groups[AddAt].GroupName);
...
function CacheGroupMember(String MemberId, String GroupId, EMcpGroupAcceptState AcceptState)
{
	...
	{
		LogInternal("("$Name$") McpGroupsManager::"$GetStateName()$":"$GetFuncName()@" Either the MemberId, GroupId, or AcceptState was not Specified.");
	}
}
...
defaultproperties
{
   ...
   Name="Default__McpGroupsManager"
   ObjectArchetype=McpGroupsBase'IpDrv.Default__McpGroupsBase'
}
