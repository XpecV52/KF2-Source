...
function OnAddMappingRequestComplete(HttpRequestInterface Request, HttpResponseInterface Response, bool bWasSuccessful)
{
	...
	if (Index != INDEX_NONE)
	{
		...
		LogInternal("Account mapping McpId(" $ AddMappingRequests[Index].McpId $ "), ExternalId(" $
...
function OnQueryMappingsRequestComplete(HttpRequestInterface Request, HttpResponseInterface Response, bool bWasSuccessful)
{
	...
	if (Index != INDEX_NONE)
	{
		...
		// Both of these need to be true for the request to be a success
		bWasSuccessful = bWasSuccessful && ResponseCode == 200;
		LogInternal("Account mapping query for ExternalType(" $
		...
		if (bWasSuccessful)
		{
			...
			if (JsonString != "")
			{
				LogInternal("JSON for account query = \r\n" $ JsonString);
...
defaultproperties
{
   ...
   Name="Default__McpIdMappingManager"
   ObjectArchetype=McpIdMappingBase'IpDrv.Default__McpIdMappingBase'
}
