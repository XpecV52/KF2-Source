...
function float GetFloatPredefinedValues(int PropertyId, int Index, float DefValue = 0)
{
	...
	if (idx == INDEX_NONE)
	{
		WarnInternal("GetFloatPredefinedValues("$PropertyId$", "$Index$"): no property");
		return DefValue;
	}
	if (FloatPredefinedValues[idx].Values.Length <= Index || Index < 0)
	{
		WarnInternal("GetFloatPredefinedValues("$PropertyId$", "$Index$"): index out of bounds");
		return DefValue;
	}
...
function string GetStringPredefinedValues(int PropertyId, int Index, string DefValue = "")
{
	...
	if (idx == INDEX_NONE)
	{
		WarnInternal("GetStringPredefinedValues("$PropertyId$", "$Index$"): no property");
		return DefValue;
	}
	if (StringPredefinedValues[idx].Values.Length <= Index || Index < 0)
	{
		WarnInternal("GetStringPredefinedValues("$PropertyId$", "$Index$"): index out of bounds");
		return DefValue;
	}
...
defaultproperties
{
   Name="Default__WebAdminSettings"
