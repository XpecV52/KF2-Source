...
function init()
{
	...
	LogInternal("Creating IWebAdminAuth instance from: "$authClass,'WebAdmin');
	...
	{
		LogInternal(""$authClass$" does not support hash algorithm "$hashLib.getAlgName(),'WebAdmin');
		hashLib = none;
    }
	...
	LogInternal("Creating ISessionHandler instance from: "$sessClass,'WebAdmin');
...
function Query(WebRequest Request, WebResponse Response)
{
	...
	if (len(SkinData) == 0)
	{
		...
		{
			response.Subst("webadminskin.name", class'WebAdminUtils'.static.HTMLEscape(Skins[i].name));
...
defaultproperties
{
   defaultAuthClass=Class'webadmin.BasicWebAdminAuth'
   defaultSessClass=Class'webadmin.SessionHandler'
   QueryHandlers(0)="WebAdmin.QHCurrentKF"
   QueryHandlers(1)="WebAdmin.QHDefaultsKF"
   QueryHandlers(2)="WebAdmin.WebAdminSystemSettings"
   startpage="/current"
   defaultDataStoreCacheClass=Class'webadmin.DataStoreCacheKF'
