...
function handleSessionBans(WebAdminQuery q)
{
	...
	if (roac != none)
	{
		...
		LogInternal("Current AccessControl class: "$webadmin.worldinfo.game.accesscontrol.class$" ; Expecting subclass of: KFAccessControl",'WebAdmin');
...
function handleSettingsPasswords(WebAdminQuery q)
{
	...
	q.response.subst("has.gamepassword", class'WebAdminUtils'.static.HTMLEscape(webadmin.WorldInfo.Game.AccessControl.RequiresPassword()));
...
function handleMapList(WebAdminQuery q)
{
	...
	q.response.subst("editmaplist.friendlyname", class'WebAdminUtils'.static.HTMLEscape(msgNewMapCycle));
 	...
 	{
 		// TODO: localize
 		q.response.subst("maplist.friendlyname", class'WebAdminUtils'.static.HTMLEscape(repl(mapList, "%i", (idx+1))$(activeidx == idx?" "$activeTag:"")));
 		q.response.subst("maplist.index", string(idx));
 		if (maplistidx == idx)
 		{
 			q.response.subst("maplist.selected", "selected=\"selected\"");
 			q.response.subst("editmaplist.friendlyname", class'WebAdminUtils'.static.HTMLEscape("Map cycle #"$(idx+1)));
 		}
 		else {
