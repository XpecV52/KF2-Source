...
function string renderLocalizedSetting(int settingId, SettingRendererState renderState)
{
	...
	if (values.length >= minOptionListSize)
	{
		for (i = 0; i < values.Length; i++)
		{
			...
			renderState.subst("setting.option.text", class'WebAdminUtils'.static.HTMLEscape(values[i].name));
		...
		for (i = 0; i < values.Length; i++)
		{
			...
			renderState.subst("setting.radio.text", class'WebAdminUtils'.static.HTMLEscape(values[i].name));
			if (values[i].id == selectedValue)
			{
	...
	for (i = 0; i < values.Length; i++)
	{
		...
		switch (values[i].Type)
		{
			...
			case SDT_String:
				...
				break;
			default:
				LogInternal("Unsupported data type "$values[i].Type$" for setting id "$settingId,'WebAdmin');
				return "";
		}
		if (selected)
		{
...
{
   minOptionListSize=4
   Name="Default__SettingsRenderer"
