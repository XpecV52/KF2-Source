	...
	if (bEnableTeamChat)
	{
		...
		for (i = 0; i < webadmin.WorldInfo.Game.GameReplicationInfo.Teams.length; i++)
		{
			...
			if (tcp != none)
			{
				...
				LogInternal("Failed to create TeamChatProxy for team "$i,'WebAdmin');
			}
		}
...
function handleCurrent(WebAdminQuery q)
{
	...
	foreach sortedPRI(pri, idx)
	{
		...
		if (cnt% 2 == 0) q.response.subst("evenodd", "even");
...
function string getCurrentTeamInfo(WebAdminQuery q)
{
	...
	if (!(webadmin.WorldInfo.Game.bTeamGame && (KFGameInfo(webadmin.WorldInfo.Game) == none || KFGameInfo(webadmin.WorldInfo.Game).teams[1] != none))) return "";
...
function handleCurrentPlayers(WebAdminQuery q)
{
	...
	foreach sortedPRI(pri, idx)
	{
		...
		if (idx% 2 == 0) q.response.subst("evenodd", "even");
		else q.response.subst("evenodd", "odd");
...
function procChatData(WebAdminQuery q, optional int startFrom, optional string substvar)
{
	...
	foreach history(entry)
	{
		...
		q.response.subst("msg.type", class'WebAdminUtils'.static.HTMLEscape(entry.type));
