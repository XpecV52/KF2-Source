...
simulated function PostBeginPlay()
{
	...
	for ( i=0; i< ReachedBehaviors.Length; i++ )
	{
		...
		{
			WarnInternal(self$" missing BehaviorArchetype at ReachedBehavior "$i);
...
simulated event IncrementCustomerCount(GameCrowdAgent ArrivingAgent)
{
	...
	if( AtCapacity() || (Queuehead != None && Queuehead.bPendingAdvance) )
	{
		...
		else
		{
			if( QueueHead != None )
			{
				WarnInternal(self$" added customer "$ArrivingAgent$" beyond capacity with queue "$QueueHead);
			}
		}
	}
	else
...
defaultproperties
{
   ...
   Begin Object Class=GameDestinationConnRenderingComponent Name=ConnectionRenderer
      ...
      ObjectArchetype=GameDestinationConnRenderingComponent'GameFramework.Default__GameDestinationConnRenderingComponent'
   End Object
   ...
   Name="Default__GameCrowdDestination"
