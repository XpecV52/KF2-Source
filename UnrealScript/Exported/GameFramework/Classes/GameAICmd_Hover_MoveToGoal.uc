...
state MoveDown extends DEBUGSTATE
{
	function vector GetMoveDest()
	{
		...
		else
		{
			...
			{
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal(GetFuncName()@"Could not find good hover height!",,);};
				Dest = Pawn.Location;
			}
		}
...
state Moving extends DEBUGSTATE
{
	...
	final function bool ReachedDest(Actor Dest)
	{
		...
		//@TONKS_TEMP
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("LatDist:"@sqrt(latDistSq),,);};
		if(latDistSq < SubGoalReachDist * SubGoalReachDist)
		{
			VertDist = abs(Pawn.Location.Z - Dest.location.Z);
			//@TONKS_TEMP
			if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("VertDist:"@VertDist,,);};
	...
	final protected function bool PopNextNode( out vector Dest )
	{
			...
			RouteCache[0] != None )
		{
			if( ReachedDest( RouteCache[0] ) )
			{
				...
				//debug
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Reached route cache 0:"@RouteCache[0],,);};
				...
				//debug
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Remove from route:"@RouteCache[0],'Move',);};
			...
			{
				//debug
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Did NOT reach route cache 0:"@RouteCache[0],,);};
...
Begin:
	...
	if( ActorReachable(Find) )
	{
		...
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Moving directly to "$Find,,);};
...
defaultproperties
{
   SubGoalReachDist=768.000000
   Name="Default__GameAICmd_Hover_MoveToGoal"
