...
function bool PickBehaviorFrom(array<BehaviorEntry> BehaviorList, optional vector BestCameraLoc=vect(0,0,0) )
{
	...
	for ( i=0; i<BehaviorList.length; i++ )
	{
		...
		{
			WarnInternal(self@MyArchetype$" No behavior archetype for behavior entry "$i);
		}
		else
		{
...
simulated function InitializeAgent( Actor SpawnLoc, const out array<CrowdSpawnerPlayerInfo> PlayerInfo, GameCrowdAgent AgentTemplate, GameCrowdGroup NewGroup, float AgentWarmUpTime, bool bWarmupPosition, bool bCheckWarmupVisibility )
{
	...
	if ( !bGroupDestination )
	{
		...
		if( SpawnDest != None )
		{
			...
			if ( CurrentDestination == None )
			{
				WarnInternal("INITIALIZING - NO CURRENTDESTINATION AFTER REACHING "$SpawnDest);
			}
...
event ActivateBehavior(GameCrowdAgentBehavior NewBehaviorArchetype, optional Actor LookAtActor )
{
	...
	{
		WarnInternal("Illegal behavior "$NewBehaviorArchetype$" for "$self);
		return;
	}
...
defaultproperties
{
   ...
   ConformTraceDist=35.000000
   ConformTraceInterval=10
   CurrentConformTraceInterval=10
   AwareRadius=256.000000
   AvoidOtherRadius=32.000000
   AvoidOtherSampleList(0)=(NumMagSamples=10)
   AvoidOtherSampleList(1)=(RotOffset=2048,NumMagSamples=8)
   AvoidOtherSampleList(2)=(RotOffset=-2048,NumMagSamples=8)
   AvoidOtherSampleList(3)=(RotOffset=4096,NumMagSamples=6)
   AvoidOtherSampleList(4)=(RotOffset=-4096,NumMagSamples=6)
   AvoidOtherSampleList(5)=(RotOffset=6144,NumMagSamples=4)
   AvoidOtherSampleList(6)=(RotOffset=-6144,NumMagSamples=4)
   AvoidOtherSampleList(7)=(RotOffset=8192,NumMagSamples=4)
   AvoidOtherSampleList(8)=(RotOffset=-8192,NumMagSamples=4)
   AvoidOtherSampleList(9)=(RotOffset=12288,NumMagSamples=2)
   AvoidOtherSampleList(10)=(RotOffset=-12288,NumMagSamples=2)
   ...
   PENALTY_COEFF_ANGLETOGOAL=2.500000
   PENALTY_COEFF_ANGLETOVEL=1.000000
   PENALTY_COEFF_MAG=1.000000
   MIN_PENALTY_THRESHOLD=0.050000
   MaxPathLaneValue=10.000000
   ExtraPathCost=50
   RotateToTargetSpeed=30000.000000
   MaxYawRate=40000.000000
   MeshMinScale3D=(X=1.000000,Y=1.000000,Z=1.000000)
   MeshMaxScale3D=(X=1.000000,Y=1.000000,Z=1.000000)
   EyeZOffset=40.000000
   ProximityLODDist=2000.000000
   VisibleProximityLODDist=5000.000000
   GroundOffset=86.000000
   SearchExtent=(X=32.000000,Y=32.000000,Z=86.000000)
   NavigationHandleClass=Class'Engine.NavigationHandle'
   WalkableFloorZ=0.700000
   NotVisibleLifeSpan=10.000000
   MaxWalkingSpeed=100.000000
   MaxRunningSpeed=300.000000
   BeaconMaxDist=1500.000000
   BeaconOffset=(X=0.000000,Y=0.000000,Z=140.000000)
   BeaconTexture=Texture2D'EngineResources.WhiteSquareTexture'
   BeaconColor=(R=0.500000,G=0.500000,B=0.500000,A=0.500000)
   RandomBehaviorInterval=30.000000
   ReachThreshold=1.000000
   DesiredGroupRadius=200.000000
   MaxLOSLifeDistanceSq=400000000.000000
   ...
   SupportedEvents(6)=Class'Engine.SeqEvent_TakeDamage'
   SupportedEvents(7)=Class'Engine.SeqEvent_Death'
   Name="Default__GameCrowdAgent"
   ObjectArchetype=CrowdAgentBase'Engine.Default__CrowdAgentBase'
}
