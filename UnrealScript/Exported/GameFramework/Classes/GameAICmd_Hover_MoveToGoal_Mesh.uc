...
function Pushed()
{
	...
	{
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Going into breadcrumb fallback state to get back onto navmesh CurLoc:"@Pawn.Location,,);};
	...
	{
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("ABORTING! Final destination"@InitialFinalDestination@"is not reachable! (ComputeValidFinalDestination returned FALSE)",,);};
		GotoState('DelayFailure');
	}
	else if( !NavigationHandle.SetFinalDestination(InitialFinalDestination) )
	{
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("ABORTING! Final destination"@InitialFinalDestination@"is not reachable! (SetFinalDestination returned FALSE)",,);};
		GotoState('DelayFailure');
	}
	else
	{
...
state MoveDown extends DEBUGSTATE
{
	...
	function vector GetMoveDest()
	{
		...
		if(NavigationHandle.LineCheck(Pawn.Location, Pawn.Location + vect(0.f,0.f,-4096.f),vect(5,5,5),HitLocation,HitNormal))
		{
			...
			if(HitActor == none)
			{
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal(GetFuncName()@"Could not find surface to adjust height to!",,);};
...
Begin:
	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Moving down!",,);};
	MoveTo(GetMoveDest());
	Sleep(1.0f);
	GotoState('Moving');
...
function bool HasReachedGoal()
{
	...
	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal(GetFuncName()@bFinalApproach@MoveToActor,,);};
...
CheckMove:
	//debug
	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("CHECKMOVE TAG",,);};
...
Begin:
	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("BEGIN TAG"@GetStateName(),,);};
	...
	else
	{
		...
		{
			//debug
			if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Couldn't generate path to location"@BP2Vect(NavigationHandle.FinalDestination)@"from"@Pawn.Location,,);};
		...
		//debug
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Generated path...",,);};
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Found path!" @ "BP2Vect(NavigationHandle.FinalDestination):'"$BP2Vect(NavigationHandle.FinalDestination)$"'",'Move',);};

		if( !NavigationHandle.GetNextMoveLocation( IntermediatePoint, SubGoalReachDist ) )
		{
			//debug
			if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Generated path, but couldn't retrieve next move location?",,);};
	...
	{
		//debug
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Still moving to"@IntermediatePoint,'Loop',);};

		bFinalApproach = VSizeSq(IntermediatePoint - BP2Vect(NavigationHandle.FinalDestination)) < 1.0;
		
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Calling MoveTo -- "@IntermediatePoint,,);};
		// if on our final move to an Actor, send it in directly so we account for any movement it does
		if( bFinalApproach && MoveToActor != None )
		{
			if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal(" - Final approach to" @ MoveToActor $ ", using MoveToward()",,);};
			Vect2BP(LastMoveTargetPathLocation, MoveToActor.GetDestination(outer));
		...
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("MoveTo Finished -- "@IntermediatePoint,,);};
...
// 				}
		...
		{
			Vect2BP(LastMoveTargetPathLocation, MoveToActor.GetDestination(outer));
			if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Repathing because target moved:" @ MoveToActor,,);};
			Goto('CheckMove');
		}
		else if( !NavigationHandle.GetNextMoveLocation( IntermediatePoint, SubGoalReachDist ) )
		{
			if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Couldn't get next move location",,);};
			...
			{
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Target is directly reachable; try direct move",,);};
			...
			{
				Sleep(0.1f);
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("GetNextMoveLocation returned false, and finaldest is not directly reachable",,);};
...
//				DrawDebugBox(Pawn.Location, vect(2,2,2) * NumMovePointFails, 255, 255, 255, true );
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("WARNING: Got same move location... something's wrong?!"@"LastMovePoint:'"$LastMovePoint$"'"@"IntermediatePoint:'"$IntermediatePoint$"'"@"Delta"@VSize(LastMovePoint-IntermediatePoint)@"ChkDist"@(Pawn.GetCollisionRadius() * 0.1f),,);};
			}
			else
			{
				NumMovePointFails=0;
			...
			{
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("ERROR: Got same move location 5x in a row.. something's wrong! bailing from this move",,);};
				Goto('FailedMove');
			}
			else
			{
				//debug
				if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("NextMove"@IntermediatePoint@"NumMovePointFails:'"$NumMovePointFails$"'",,);};
			}
		}
	}

	//debug
	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Reached end of move loop??",,);};

	Goto( 'CheckMove' );

FailedMove:

	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Failed move.  Now ZeroMovementVariables",,);};
...
ReachedGoal:
	//debug
	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Reached move point:"@BP2Vect(NavigationHandle.FinalDestination)@VSize(Pawn.Location-BP2Vect(NavigationHandle.FinalDestination)),,);};
...
Begin:
if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("trying to move back along breadcrumb path",,);};
	if( NavigationHandle.GetNextBreadCrumb(IntermediatePoint) )
	{
		if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Moving to breadcrumb pos:"$IntermediatePoint,,);};
...
// 		}
		...
		if( poses.length > 0)
		{
			...
			if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Retval:'"$Retval$"'",,);};
...
Begin:
	...
	if( ! class'Engine'.static.GetEngine().bDisableAILogging) {AILog_Internal("Fallback! We now try MoveTo directly to a point that is avail to us",,);};
...
defaultproperties
{
   SubGoalReachDist=128.000000
   MaxMovePointFails=5
   Name="Default__GameAICmd_Hover_MoveToGoal_Mesh"
