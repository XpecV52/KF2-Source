...
simulated event ReachedDestination(GameCrowdAgent Agent)
{
		...
		for ( QueuePoint = Agent.CurrentDestination.QueueHead; QueuePoint!=None; QueuePoint= QueuePoint.NextQueuePosition)
		{
			if ( QueuePoint.NextQueuePosition == self )
			{
				...
				{
					WarnInternal(agent$"in queue behind empty spot at "$self);
				}
...
private simulated function ActuallyAdvance()
{
	...
	if ( QueuedAgent != None )
	{
		...
		else
		{
			...
			if ( QueueFront == None )
			{
				WarnInternal("Illegal front position for queue "$self);
				return;
			}
		...
		if ( QueuedAgent != None )
		{
			WarnInternal(self$" GOT QUEUED AGENT BACK - Head "$PreviousQueuePosition$" Tail "$NextQueuePosition);
		}
...
simulated function AddCustomer(GameCrowdAgent NewCustomer, GameCrowdInteractionPoint PreviousPosition)
{
	...
	{
		WarnInternal(self$" Attempted to add customer "$NewCustomer$" beyond end of queue");
	}
}
...
simulated function ClearQueue(GameCrowdAgent OldCustomer)
{
	if ( !bClearingQueue )
	{
		...
		{
			WarnInternal("Attempted to clear "$OldCustomer$" from queue position with customer "$QueuedAgent);
		}
		bClearingQueue = false;
	}
}
...
defaultproperties
{
   ...
   Name="Default__GameCrowdDestinationQueuePoint"
