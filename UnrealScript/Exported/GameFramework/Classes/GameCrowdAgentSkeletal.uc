...
simulated function CreateAttachments()
{
	...
	for(AttachIdx=0; AttachIdx < Attachments.length; AttachIdx++ )
	{
		...
		if( Attachments[AttachIdx].List[PickedInfoIdx].StaticMesh != None )
		{
			...
			{
				LogInternal("CrowdAgent: WARNING: Could not find socket or bone called '"$Attachments[AttachIdx].SocketName$"' for mesh '"@Attachments[AttachIdx].List[PickedInfoIdx].StaticMesh$"'");
			}
		}
	}
}
...
defaultproperties
{
   ...
   AnimVelRate=0.009800
   MaxSpeedBlendChangeSpeed=2.000000
   MoveSyncGroupName="MoveGroup"
   MaxTargetAcquireTime=5.000000
   bAllowSkeletonUpdateChangeBasedOnTickResult=True
   NotVisibleDisableTickTime=0.200000
   MaxAnimationDistance=12000.000000
   ...
   Name="Default__GameCrowdAgentSkeletal"
