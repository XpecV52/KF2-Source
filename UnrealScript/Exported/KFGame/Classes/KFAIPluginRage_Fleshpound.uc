...
function bool CanDoRage()
{
	...
	if( Enemy != none && RageFrustrationStartTime > 0 && (WorldInfo.TimeSeconds - RageFrustrationStartTime) > RageFrustrationTime )
	{
		return true;
	}
...
function StartRageTimer(optional bool bJustSawEnemy)
{
	...
	if( bJustSawEnemy && (WorldInfo.TimeSeconds - LastEnemySightedTime) > MaxEnemyNotVisibleRageTime )
	{
...
defaultproperties
{
   MaxEnemyNotVisibleRageTime=30.000000
   RageFrustrationBaseTime=9.000000
   RageDamageThreshold=276.000000
   Name="Default__KFAIPluginRage_Fleshpound"
