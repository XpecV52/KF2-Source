...
protected function bool ShouldApply(EInstantAfflictions Id, EHitZoneBodyPart BodyPart, int InPower, optional float CooldownModifier=1.0, optional bool bTestOnly)
{
	...
	{
		if (bDebugLog) LogInternal(Id@self@"Resistance is Base:"@BaseResistValue@"LowHealthBonus:"@InstantAffl[Id].LowHealthBonus@"Power: "$InPower,'StatusEffect');

		if ( (WorldInfo.TimeSeconds - InstantAffl[Id].LastStartTime) < (InstantAffl[Id].Cooldown*CooldownModifier) )
		{
...
function bool AccrueStackingAffliction(EStackingAfflictions Id, float InPower)
{
	...
	if ( (WorldInfo.TimeSeconds - StackingAffl[Id].LastStartTime) < StackingAffl[Id].Cooldown )
	{
	...
	if (bDebugLog) LogInternal(GetFuncName()@Id@"Threshold:"$StackingAffl[Id].Threshhold@"Power:"$StackingAffl[Id].StackedPower);
