...
function bool CanDoAttackAnim(int Idx, KFPawn P, optional Actor Target)
{
	...
	if ( Attacks[Idx].GlobalCooldown > 0 && Attacks[Idx].LastTimePlayed > 0.f && ((P.WorldInfo.TimeSeconds - Attacks[Idx].LastTimePlayed) < Attacks[Idx].GlobalCooldown) )
	{
		if (bDebugLog) LogInternal(P@GetFuncName()$"() rejecting attack idx "$idx$" because GlobalCooldown ("$Attacks[Idx].GlobalCooldown$") LastTimePlayed:"$(P.WorldInfo.TimeSeconds - Attacks[Idx].LastTimePlayed));
		return false;
	}
	if ( Attacks[Idx].Chance < 1.f && FRand() > Attacks[Idx].Chance )
	{
		if (bDebugLog) LogInternal(P@GetFuncName()$"() rejecting attack idx "$idx$" because random chance missed (Chance:"$Attacks[Idx].Chance$")");
		return false;
	}
	...
	if ( Target != None )
	{
		...
		{
			if (bDebugLog) LogInternal(P@GetFuncName()$"() rejecting attack idx "$idx$" because target dist ("$VSize(P.Location - Target.Location)$") is greater than "$Attacks[Idx].MaxDistance);
			return false;
		}
		if ( Attacks[Idx].MinDistance > 0 && VSizeSq(P.Location - Target.Location) < Square(Attacks[Idx].MinDistance) )
		{
			if (bDebugLog) LogInternal(P@GetFuncName()$"() rejecting attack idx "$idx$" because target dist is less than "$Attacks[Idx].MinDistance);
			return false;
		}
	}
	...
	{
		if (bDebugLog) LogInternal(P@GetFuncName()$"() rejecting attack idx "$idx$" because target is not surrounded");
		return false;
	}
	...
	if ( !AllowAttackByDifficulty(Idx) )
	{
		if( P.MyKFAIC != none )
		{
			if( P.MyKFAIC!= None ) { P.MyKFAIC.AILog_Internal(GetFuncName()$"() PREVENTING attack "$Attacks[idx].Tag$" because it has a DifficultyRating of "$Attacks[idx].DifficultyRating,'Command_Attack_Melee'); };
		}
