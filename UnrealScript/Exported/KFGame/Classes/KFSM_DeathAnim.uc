...
function bool PlayDeathAnimation(class<DamageType> DamageType, vector HitDirection, optional name HitBoneName)
{
	...
	if (KFPOwner.bLogCustomAnim) LogInternal("PlayDeathAnimation success DamageType:"$DamageType@"BoneName:"$HitBoneName);
...
function SpecialMoveEnded( Name PrevMove, Name NextMove )
{
	...
	if (KFPOwner.bLogCustomAnim) LogInternal("DeathAnimation finished ChainCount:"$NumChainedDeathAnims);
}
...
final function SetSpringForBone(Name InBoneName, bool bEnable)
{
	...
	if( InBoneName == '' )
	{
		WarnInternal(PawnOwner.WorldInfo.TimeSeconds @ PawnOwner @ class @ GetFuncName() @ "Has non existing Bone or Socket named:" @ InBoneName @ "Unable to create HipBodyInstance!");
		return;
	}
	...
	if( BoneIndex != INDEX_NONE )
	{
		...
		{
			WarnInternal(PawnOwner.WorldInfo.TimeSeconds @ PawnOwner @ class @ GetFuncName() @ "BodyInstance not found for BoneName:" @ InBoneName);
			ScriptTrace();
		}
	}
	else
	{
		WarnInternal(PawnOwner.WorldInfo.TimeSeconds @ PawnOwner @ class @ GetFuncName() @ "Has non existing Index for BoneName:" @ InBoneName @ "Unable to create BodyInstance!");
	}
}
