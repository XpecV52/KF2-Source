...
reliable server final function ServerRemoveFromInventory(Inventory ItemToRemove)
{
   	if (Role == ROLE_Authority)
    {
		...
		if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("sell", Instigator.PlayerReplicationInfo, ItemToRemove.Class);
	}
}
...
reliable server function ServerCloseTraderMenu()
{
	...
	for(i = TransactionItems.Length - 1; i >= 0; i--)
	{
		...
		if( KFWClass != none )
		{
			...
			if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("buy", Instigator.PlayerReplicationInfo, KFWClass);
		}
		...
		if( GetTraderItemFromWeaponLists(WeaponItem, ItemIndex) )
		{
			...
			if( GetWeaponFromClass(KFW, WeaponItem.ClassName) )
			{
				if( bSecondaryAmmo )
				{
					...
					if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("buy", Instigator.PlayerReplicationInfo, "S.ammo", KFW.class, "#"$AmountPurchased);
				}
				else
				{
					...
					if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("buy", Instigator.PlayerReplicationInfo, "ammo", KFW.class, "#"$AmountPurchased);
		    	}
			}
			else
			{
		...
		if( GetTraderItemFromWeaponLists(WeaponItem, ItemIndex) )
		{
			...
			if( TransactionIndex != INDEX_NONE )
			{
				...
				if ( bSecondaryAmmo )
				{
					...
					if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("buy", Instigator.PlayerReplicationInfo, "S.ammo", WeaponItem.ClassName, "#"$AmountAdded);
				}
				else
				{
					...
					if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("buy", Instigator.PlayerReplicationInfo, "ammo", WeaponItem.ClassName, "#"$AmountAdded);
				}
			}
		}
	}
...
reliable server final private function ServerBuyArmor( float PercentPurchased )
{
	...
	if( Role == ROLE_Authority && KFP != none && bServerTraderMenuOpen )
	{
		if( ProcessArmorDosh( PercentPurchased ) )
		{
			...
			if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("buy", Instigator.PlayerReplicationInfo, "armor", "#"$PercentPurchased);
		}
	}
}
...
reliable server final private function ServerBuyGrenade( int AmountPurchased )
{
    if ( Role == ROLE_Authority && bServerTraderMenuOpen)
    {
		if(ProcessGrenadeDosh(AmountPurchased))
		{
			...
			if(WorldInfo.GRI != none && WorldInfo.GRI.GameClass.static.AllowAnalyticsLogging()) WorldInfo.TWLogEvent ("buy", Instigator.PlayerReplicationInfo, "grenades", "#"$AmountPurchased);
    	}
    }
}
...
reliable server final function ServerSellWeapon( byte ItemIndex )
{
    ...
    if (Role == ROLE_Authority && bServerTraderMenuOpen)
    {
		...
		if( KFPRI != none && GetTraderItemFromWeaponLists(SoldItem, ItemIndex) )
		{
	    	...
	    	{
				if (bLogInventory) LogInternal("ServerSellWeapon: Calling ServerRemoveFromInventory on" @ SoldItem.ClassName);
	    	...
	    	else   // Otherwise it's a transaction item that needs to be removed
	    	{
				...
				if (bLogInventory) LogInternal("ServerSellWeapon: SoldItem="$SoldItem.ClassName @ "TransactionIndex="$TransactionIndex);
				if( TransactionIndex != INDEX_NONE )
				{
					...
					if (bLogInventory) LogInternal("ServerSellWeapon: Calling RemoveTransactionItem on" @ SoldItem.ClassName);
...
final function RemoveTransactionItem( const out STraderItem ItemToRemove )
{
	...
	if (bLogInventory) LogInternal("RemoveTransactionItem: ItemToRemove="$ItemToRemove.ClassName);
	if( Role < ROLE_Authority || !bServerTraderMenuOpen )
	{
		return;
	}
	...
	if (bLogInventory) LogInternal("RemoveTransactionItem: TransactionIndex="$TransactionIndex);
	if (bLogInventory)
	{
...
private final function bool ProcessWeaponDosh(out STraderItem PurchasedItem)
{
	...
	if (bLogInventory) LogInternal("Server failed to process " @PurchasedItem.ClassName);
	return false;
}
...
private final function bool ProcessAmmoDosh(out STraderItem PurchasedItem, int AdditionalAmmo, optional bool bSecondaryAmmo)
{
	...
	if (bLogInventory) LogInternal("Server failed to process " @PurchasedItem.ClassName @"Ammo");
	return false;
}
...
simulated function bool GetIsOwned( name ClassName )
{
	...
	ForEach InventoryActors(class'Inventory', Inv)
	{
		...
		{
			if (bLogInventory) LogInternal("GetIsOwned: ClassName="$ClassName @ "InventoryActors true");
			return true;
		}
	}
	...
	for( i = 0; i < TransactionItems.length; ++i )
	{
		if( TransactionItems[i].ClassName == ClassName )
		{
			if (bLogInventory) LogInternal("GetIsOwned: i="$i @ "ClassName="$ClassName @ "TransactionItems true");
			return true;
		}
	}

	if (bLogInventory) LogInternal("GetIsOwned: ClassName="$ClassName @ "false");
	return false;
}
