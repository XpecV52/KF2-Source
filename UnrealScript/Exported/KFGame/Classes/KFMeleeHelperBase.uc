...
simulated function float RateMeleeVictim(Pawn Victim, vector StartTrace, vector EndTrace, float Range, float FOVCosine, optional float RatingToBeat)
{
	...
	if (bLogMelee) LogInternal("Melee considering:"@Victim);
	...
	{
		if (bLogMelee) LogInternal("rejected within vertical melee range");
		return -1.f;
	}
	...
	{
		if (bLogMelee) LogInternal("rejected victim with lower rating");
		return -1.f;
	}
	...
	if( FOVCosine > 0.f )
	{
		...
		{
            if (bLogMelee) LogInternal("rejected:"@Victim@"dot:"@DirToVictim dot AimDir);
			return -1.f;
		}
		else
		{
            if (bLogMelee) LogInternal("accepted:"@Victim@"dot:"@DirToVictim dot AimDir);
		}
	}
	...
	{
		if (bLogMelee) LogInternal("rejected:"@Victim@"melee obstruction: ");
		return -1.f;
	}
...
event ProcessHitboxCollision(Actor HitActor, vector StartTrace, vector EndTrace, vector HitLocation, vector HitNormal, const out TraceHitInfo HitInfo, optional out ImpactInfo Impact)
{
	...
	if (bLogMelee) LogInternal(GetFuncName()@HitActor@HitInfo.Item@HitInfo.BoneName);
}
...
simulated function PlayMeleeHitEffects(Actor Target, vector HitLocation, vector HitDirection, optional bool bShakeInstigatorCamera=true)
{
	...
	else if( Instigator.IsHumanControlled() && Instigator.IsLocallyControlled() ) // Local client impacts
	{
		...
		else if( !Target.IsA('Pawn') ) // Pawns will handle impacts in PlayHit()
		{
			...
			KFImpactEffectManager(WorldInfo.MyImpactEffectManager).PlayImpactEffects( HitLocation, Instigator, HitDirection,,, true );		
		}
	}
}
...
defaultproperties
{
   MaxHitRange=150.000000
   DefaultFOVCosine=0.600000
   HitboxSpacing=30.000000
   HitboxBoneName="RW_Damage"
   HitboxBoneAxis=AXIS_Z
   bDoHitboxObstructionTrace=True
   bAllowMeleeToFracture=True
   MeleeVictimCamShake=CameraShake'KFGame.Default__KFMeleeHelperBase:MeleeImpactCamShake0'
   MeleeImpactCamShake=KFCameraShake'FX_CameraShake_Arch.Melee.Default_Melee'
   Name="Default__KFMeleeHelperBase"
