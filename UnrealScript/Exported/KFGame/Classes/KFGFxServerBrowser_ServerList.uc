...
function OnFindOnlineGamesCompleteDelegate(bool bWasSuccessful)
{
	...
	LogInternal("OnFindOnlineGamesCompleteDelegate called!, bWasSuccessful is" @ bWasSuccessful);
	...
	if ( !bSearchCompleted )
	{
		...
		{
			class'WorldInfo'.static.GetWorldInfo().TimerHelper.SetTimer( 0.01, false, nameof(UpdateListDataProvider), self );
		}
		...
		for (i = 0; i < LatestGameSearch.Results.Length; i++)
		{
			if ( LatestGameSearch.Results[i].GameSettings.GfxId == INDEX_NONE || LatestGameSearch.Results[i].GameSettings.GfxId >= GFxServerObjects.length  )
			{
				...
				if (class'WorldInfo'.static.IsEOSBuild() && TempOnlineGamesSettings.PingInMs == -1) {
					class'WorldInfo'.static.GetWorldInfo().TimerHelper.SetTimer( 0.1, false, nameof(UpdateListDataProvider), self );
					break;
				}
				//@SABER_EGS_END
