...
simulated event DoSpecialMove(ESpecialMove NewMove, optional bool bForceMove, optional Pawn InInteractionPawn, optional INT InSpecialMoveFlags, optional bool bSkipReplication, optional bool bIsPendingMove)
{
	...
	if (bLogSpecialMove) LogInternal("New special move requested:" @ NewMove @ InInteractionPawn @ InSpecialMoveFlags @ "bForceMove:" @ bForceMove);
	...
	if( bIsPendingMove )
	{
		if (bLogSpecialMove) LogInternal("- PendingSpecialMove is being started.");
	...
	if ( !bIsPendingMove )
	{
		...
		{
			if (bLogSpecialMove) LogInternal("- leaving move:" @ SpecialMove);
...
simulated private function bool CanOverrideSpecialMove(ESpecialMove NewMove, Pawn InInteractionPawn, int InSpecialMoveFlags)
{
	...
	{
		if (bLogSpecialMove) LogInternal("- Overriding" @ SpecialMove @ "with" @ NewMove @ "(previous pending:" @ PendingSpecialMoveStruct.SpecialMove $ ")");
		return true;
	}
	else
	{
		...
		{
			if (bLogSpecialMove) LogInternal("- chaining" @ NewMove @ "after" @ SpecialMove @ "(previous pending:" @ PendingSpecialMoveStruct.SpecialMove $ ")");
		...
		{
			WarnInternal(WorldInfo.TimeSeconds @ Self @ GetFuncName() @ "Cannot override, cannot chain." @ NewMove @ "is lost! SpecialMove:" @ SpecialMove @ "Pending:" @ PendingSpecialMoveStruct.SpecialMove);
		}
		return false;
	}
}
...
simulated private function bool DoPendingSpecialMove(bool bForceMove)
{
	...
	if (bLogSpecialMove) LogInternal("- triggering pending special move:" @ PendingSpecialMoveStruct.SpecialMove);
	...
	{
		if (bLogSpecialMove) LogInternal("- PendingSpecialMove trigger successful for" @ NextMove);
		// success, we can leave
		return true;
	}
	else
	{
		if (bLogSpecialMove) LogInternal("- PendingSpecialMove trigger failed for" @ NextMove @ "Proceeding to clear SpecialMove");
		return false;
	}
}
...
simulated private function ReassignSpecialMove(Pawn InInteractionPawn, int InSpecialMoveFlags, bool bSkipReplication)
{
	...
	{
		if (bLogSpecialMove) LogInternal("- updating special move flags");
		SpecialMoveFlags = InSpecialMoveFlags;
	...
	{
		if (bLogSpecialMove) LogInternal("- updating interaction pawn");
		InteractionPawn = KFPawn(InInteractionPawn);
	...
	if (bLogSpecialMove) LogInternal("- ignoring redundant call");
}

/** helper for DoSpecialMove */
simulated private function ReplicateSpecialMove(ESpecialMove NewMove, Pawn InInteractionPawn, int InSpecialMoveFlags)
{
	if (bLogSpecialMove) LogInternal("- Replicating New SM" @ SpecialMove);
...
simulated final private function SpecialMoveStarted(ESpecialMove NewMove, ESpecialMove PrevMove, bool bForced)
{
	...
	if (bLogSpecialMove) LogInternal("NewMove:" @ NewMove @ "PrevMove:" @ PrevMove);
...
simulated final function SpecialMoveEnded(ESpecialMove PrevMove, ESpecialMove NextMove)
{
	...
	if (bLogSpecialMove) LogInternal("PrevMove:" @ PrevMove);
...
{
	if (bLogSpecialMove) LogInternal("- Clearing PendingSpecialMove:");
...
{
	if (bLogSpecialMove) LogInternal("SpecialMoveToEnd:" @ SpecialMoveToEnd);
...
defaultproperties
{
   Name="Default__KFSpecialMoveHandler"
