...
function bool RegisterPointLight(PointLightComponent LightComp, optional LightPoolPriority PriorityType=LPP_Low, optional bool bForceAllowReenable)
{
	...
	{
		LogInternal(GetFuncName()@"failed because light has not been attached");
	...
	{
		LogInternal(GetFuncName()@"perf warning: dedicated server shouldn't be playing with lights on"@LightComp.Owner);
	...
	{
		LogInternal(GetFuncName()@"failed because bEnabled == FALSE.  Unsupported with bForceAllowReenable");
		LightComp.DetachFromAny();
		return false;
	}
	...
	{
		if (bDebug) LogInternal(GetFuncName()@"disabling incoming light for Owner:"$LightComp.Owner@"Priority:"$NewLight.Priority@"Total:"$ActiveComponents.Length);
	...
	if (bDebug) LogInternal(GetFuncName()@"adding light for Owner:"$LightComp.Owner@"Priority:"$NewLight.Priority@"Total:"$ActiveComponents.Length);
