...
exec function SetInventoryFilter (string FilterType, int NewEnum)
{
    ...
    LogInternal("Set Filter Called: " @FilterType @NewEnum);
...
exec function PrintOutCharacterCosmeticInfo()
{
    ...
    {
        LogInternal("Item on slot index: " @i @"Mesh index: " @KFPRI.RepCustomizationInfo.AttachmentMeshIndices[i] @"Skin index" @KFPRI.RepCustomizationInfo.AttachmentSkinIndices[i]);
    }
...
exec function LogCurrentWave()
{
    ...
    if( KFGameInfo(WorldInfo.Game) != none )
	{
        ...
        for( i = 0; i < SpawnManager.AvailableSquads.Length; i++ )
    	{
    		...
    		{
                LogInternal("AvailableSquads "$i$" MonsterList "$j$" is "$GetEnum(enum'EAIType',SpawnManager.AvailableSquads[i].MonsterList[j].Type)$" Num: "$SpawnManager.AvailableSquads[i].MonsterList[j].Num$" SquadName: "$SpawnManager.AvailableSquads[i]);
    		}
    	}

        LogInternal("Current Waves Info For Wave: "$KFGameInfo(WorldInfo.Game).MyKFGRI.WaveNum$" which is "$(KFGameInfo(WorldInfo.Game).MyKFGRI.WaveNum - 1)$" in the wave array ");
...
simulated exec function TestHitZones()
{
	...
	foreach Pawn.TraceActors( class'Actor', TestActor, HitLocation, HitNormal, TraceEnd, TraceStart,,CurrentImpact, TRACEFLAG_Bullet )
	{
		...
		{
            LogInternal("Hit this: "$TestActor$" "$CurrentImpact.HitComponent$" "$CurrentImpact.BoneName);
		}
	}
}
...
{
	if (true) LogInternal("--------------------------------------------------------------------------------------------");
	if (true) LogInternal("AI Debug Commands");
	if (true) LogInternal("--------------------------------------------------------------------------------------------");
	if (true) LogInternal("	AIDEBUG					Makes AI non-aggressive and ready to receive debug commands");
	if (true) LogInternal(" AISETCANATTACK True/False Sets whether the debug mode pawn should be capable of attacking you");
	if (true) LogInternal("	TOGGLEALLNPCDEBUG		Renders names, some vital info, and a health bar above NPC heads");
	if (true) LogInternal("	AISTEPASIDE				Forces StepAside AI Command");
	if (true) LogInternal("	AITOGGLEHEADTRACKING	Toggles AI HeadTracking");
	if (true) LogInternal("	AIDEBUGTURNINPLACE		Starts TurnInPlace debug command - AI will constantly turn to player");
	if (true) LogInternal("	AITAUNT					Forces AI to execute a taunt command");
	if (true) LogInternal("	AIPATHTO <name>			AI will attempt to move/path to KFPathnode with DebugTagName matching <name>");
	if (true) LogInternal("--------------------------------------------------------------------------------------------");
}
...
exec function ShowBlockedPaths( optional bool bSkipWalls=true, optional bool bRenderResult=true, optional bool bLogResult=true, optional float WithinPawnRadius=0.f )
{
	...
	if( KFMP != none && KFMP.MyKFAIC != none )
	{
		...
		{
			LogInternal("===============================================================================");
			LogInternal("Blocked paths for "$KFMP$" <"$KFMP.MyKFAIC$"> at "$KFMP.Location$" [NPC Radius: "$KFMP.GetCollisionRadius()$" Height: "$KFMP.GetCollisionHeight()$"] Time: "$WorldInfo.TimeSeconds);
			LogInternal(" ");
		}
		...
		foreach WorldInfo.AllNavigationPoints( class'NavigationPoint', N )
		{
			...
			if( bLogResult )
			{
				LogInternal("   Checking "$N);
				LogInternal("    bBlocked:"$N.bBlocked$" bOneWayPath:"$N.bOneWayPath$" ExtraCost: "$N.ExtraCost);
				if( N.AnchoredPawn != none )
				{
					LogInternal("    AnchoredPawn:"$N.AnchoredPawn$" LastAnchoredPawnTime:"$N.LastAnchoredPawnTime);
				}
			}
			...
			for( SpecIdx = 0; SpecIdx < N.PathList.Length; SpecIdx++ )
			{
				...
				if( bInBlockedPathList || N.bBlocked || (Cost >= N.PathList[SpecIdx].BLOCKEDPATHCOST) )
				{
					...
					if( bLogResult )
					{
						...
						LogInternal("  Found blocked path for "$KFMP$" in NavPoint "$N$" [bBlocked:"$N.bBlocked$"][OneWay:"$N.bOneWayPath$"][ExtraCost:"$N.ExtraCost$"]");
						if( N.AnchoredPawn != none )
						{
							LogInternal("   "$N$"'s anchored pawn is "$N.AnchoredPawn);
						}
						...
						LogInternal("		  Max Radius:"$N.PathList[SpecIdx].CollisionRadius$" Max Height:"$N.PathList[SpecIdx].CollisionHeight$" MaxLandingVelocity:"$N.PathList[SpecIdx].MaxLandingVelocity$" bCanCutCorners:"$N.PathList[SpecIdx].bCanCutCorners);
						for( RouteCacheIdx = 0; RouteCacheIdx < KFAIC.RouteCache.Length; RouteCacheIdx++ )
						{
							if( KFAIC.RouteCache[RouteCacheIdx] == N )
							{
								LogInternal("		  Found "$N$" in "$KFAIC$"'s routecache [Slot "$RouteCacheIdx$"]");
								break;
							}
						}
					}
		...
		{
			LogInternal(" ");
			LogInternal("	  Found "$BlockedCount$" blocked paths (out of "$Total$" checked) for "$KFMP);
			LogInternal("===============================================================================");
		}
	}
}
...
exec function DumpDebugNodes()
{
	...
	foreach WorldInfo.AllActors( class'KFPathnode', KFPN )
	{
		if( KFPN.DebugTag != '' )
		{
			LogInternal("Found "$KFPN$" with DebugTag "$KFPN.DebugTag$" at "$KFPN.Location);
		}
	}
}
...
exec function ToggleShowVisualStuckZedDebugInfo()
{
	...
	foreach AllActors( class'KFAIController', KFAIC )
	{
		...
		LogInternal("ToggleShowVisualStuckZedDebugInfo - " @ KFAIC.bShowVisualStuckZedDebugInfo);
...
exec function ToggleShowMovePointsDebugInfo()
{
	...
	foreach AllActors( class'KFAIController', KFAIC )
	{
		...
		LogInternal("ToggleShowMovePointsDebugInfo - " @ KFAIC.bShowMovePointsDebugInfo);
...
exec function ToggleShowCollisionRadiusForReducedZedOnZedPinchPointCollisionState()
{
	...
	foreach AllActors( class'KFAIController', KFAIC )
	{
		...
		LogInternal("ToggleShowCollisionRadiusForReducedZedOnZedPinchPointCollisionState - " @ KFAIC.bShowCollisionRadiusForReducedZedOnZedPinchPointCollisionState);
	}
}
...
exec function ToggleShowHighDetailCombatMovementDebugInfo()
{
	...
	foreach AllActors( class'KFAIController', KFAIC )
	{
		...
		LogInternal("ToggleShowVisualStuckZedDebugInfo - " @ KFAIC.bShowHighDetailCombatMovementDebugInfo);
...
exec function ToggleShowLeapDownDebugArtifacts()
{
	...
	foreach AllActors( class'KFAIController', KFAIC )
	{
		...
		LogInternal(GetFuncName() @ " - " @ KFAIC.bShowLeapDownDebugArtifacts);
...
exec function ToggleShowDoorNavigationDebugArtifacts()
{
	...
	foreach AllActors( class'KFAIController', KFAIC )
	{
		...
		LogInternal(GetFuncName() @ " - " @ KFAIC.bShowDoorNavigationDebugArtifacts);
...
exec function ToggleShowDestructibleNavigationDebugArtifacts()
{
	...
	foreach AllActors( class'KFAIController', KFAIC )
	{
		...
		LogInternal(GetFuncName() @ " - " @ KFAIC.bShowDestructibleNavigationDebugArtifacts);
...
exec function CheckReachSpecs()
{
	...
	foreach WorldInfo.AllNavigationPoints( class'NavigationPoint', NP )
	{
		for( i = 0; i < NP.PathList.Length; i++ )
		{
			...
			{
				LogInternal(NP$" FOUND NO END FOR REACHSPEC "$NP.PathList[i]$" in slot "$i);
			}
		...
		for( x = 0; x < NP.PathList.Length; x++ )
		{
			...
			for( i = 0; i < Templist.Length; i++ )
			{
				if( NavigationPoint( NP.PathList[x].End.Actor ) == Templist[ i ] )
				{
					if( iCount > 0 )
					{
						LogInternal("FOUND DUPE REACHSPEC ("$NP.PathList[x]$" IN "$NP$" LEADING TO "$Templist[ i ]);
					}
					iCount++;
				}
			}
...
exec function DumpReachSpecs()
{
		...
		for( i = 0; i < Nav.PathList.Length; i++ )
		{
			...
			if( !Nav.PathList[i].IsA( 'ProscribedReachSpec' ) )
			{
				...
				if( StartNav != none && EndNav!= none )
				{
					...
					LogInternal("		"$Nav.PathList[i]$" connected to "$EndNav$" "$Length$" length");
					TotalDist += Length;
				}
			}
		}
	...
	LogInternal("Avg Dist: "$TotalDist / Count);
}
	...
	if ( KFPC != none )
	{
		...
		ForEach WorldInfo.AllPawns(class'KFPawn_Monster', AIP)
		{
			...
			if( KillDistance > 0.f )
			{
				...
				{
					LogInternal(self$" Not killing "$AIP$" who is "$VSize( AIP.Location - MyPawn.Location)$" units away");
					continue;
				}
			}
...
exec function TestSpecDist( optional bool bOnlyPathNodes = false)
{
	...
	{
		LogInternal("DISTANCES["$i$"] : "$Distances[ i ]);
		Total += Distances[ i ];
	}

	LogInternal("TOTAL: "$Total$" LENGTH: "$Distances.Length$" AVG: "$Total/Distances.Length);
}
...
simulated function KFPawn SpawnAIZed(string ZedName, float Distance, optional name SpawnAtTag)
{
	...
	LogInternal(GetFuncName()$"() Spawned: "$Zed);
...
exec function TestSpawnVolume( int SpawnAmount, optional bool bCrawlersOnly, optional bool bBloatsOnly )
{
	...
	for( i = 0; i < SpawnAmount; i++ )
	{
		LogInternal("FakeSpawnList "$i$" is "$FakeSpawnList[ i ]);
	}
...
exec function ConvertFOV(float InFOV)
{
	...
	LogInternal("FOV --- Original:"@InFOV@"New:"@NewFOV);
}
...
exec function TestFalloffDamage(float Distance, float MaxRadius, float FalloffExponent)
{
	...
	LogInternal(GetFuncName()@"Result:"$DamageScale);
}
...
{
	LogInternal("Current KFGame version:" @ class'KFGameEngine'.static.GetKFGameVersion());
}
    ...
    if( KFGI != none )
    {
        ...
        for( i = 0; i < KFGI.AmmoPickups.Length; i++ )
        {
            ...
            if( KFGI.AmmoPickups[i].IsInState( 'Disabled' ) )
            {
                ...
                LogInternal("~~LogAmmoPickups: AmmoPickup" @ i @  "is disabled! Location:" @ KFGI.AmmoPickups[i].Location);
            }
            else if( KFGI.AmmoPickups[i].IsInState( 'sleeping' ) )
            {
                NumSleepingAmmoPickups += 1.f;
                LogInternal("~~LogAmmoPickups: AmmoPickup" @ i @  "is sleeping! Location:" @ KFGI.AmmoPickups[i].Location @ "; Time until respawn: " @ KFGI.AmmoPickups[i].GetRemainingTimeForTimer(nameof(KFGI.AmmoPickups[i].Reset)));
            }
            else if( KFGI.AmmoPickups[i].IsInState( 'pickup' ) )
            {
                NumActiveAmmoPickups += 1.f;
                LogInternal("~~LogAmmoPickups: AmmoPickup" @ i @  "is ACTIVE! Location:" @ KFGI.AmmoPickups[i].Location);
            }
        }
        LogInternal("~~LogAmmoPickups ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        LogInternal("~~LogAmmoPickups: Total num ammo pickup ups:   " @ Round( NumTotalAmmoPickups ));
        LogInternal("~~LogAmmoPickups: Disabled ammo pickup ups:    " @ Round( NumDisabledAmmoPickups ));
        LogInternal("~~LogAmmoPickups: Sleeping ammo pickup ups:    " @ Round( NumSleepingAmmoPickups ));
        LogInternal("~~LogAmmoPickups: Active ammo pickup ups:      " @ Round( NumActiveAmmoPickups ));
        LogInternal("~~LogAmmoPickups: Percentages: Disabled:" @ (100 / NumTotalAmmoPickups) * NumDisabledAmmoPickups $ "%" @ "Sleeping:" @ (100 / NumTotalAmmoPickups) * NumSleepingAmmoPickups $ "%" @ "Active:" @ (100 / NumTotalAmmoPickups) * NumActiveAmmoPickups $ "%");
       ...
       LogInternal("~~LogItemPickups ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        for( i = 0; i < KFGI.ItemPickups.Length; i++ )
        {
            ...
            if( KFGI.ItemPickups[i].IsInState( 'Disabled' ) )
            {
                ...
                LogInternal("~~LogItemPickups: ItemPickup" @ i @  "is disabled! Location:" @ KFGI.ItemPickups[i].Location);
            }
            else if( KFGI.ItemPickups[i].IsInState( 'sleeping' ) )
            {
                NumSleepingItemPickups += 1.f;
                LogInternal("~~LogItemPickups -------------------------------------");
                LogInternal("~~LogItemPickups: ItemPickup" @ i @  "is sleeping! Location:" @ KFGI.ItemPickups[i].Location);
            }
            else if( KFGI.ItemPickups[i].IsInState( 'pickup' ) )
            {
                NumActiveItemPickups += 1.f;
                LogInternal("~~LogItemPickups -------------------------------------");
                LogInternal("~~LogItemPickups: ItemPickup" @ i @  "is ACTIVE! Location:" @ KFGI.ItemPickups[i].Location);
                LogInternal("~~LogItemPickups: ItemPickup" @ i @  "item is a" @ KFPickupFactory_Item(KFGI.ItemPickups[i]).ItemPickups[ KFPickupFactory_Item(KFGI.ItemPickups[i]).PickupIndex ].ItemClass.name);
            }
        }
        LogInternal("~~LogItemPickups ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        LogInternal("~~LogItemPickups: Total num item pickups:   " @ Round( NumTotalItemPickups ));
        LogInternal("~~LogItemPickups: Disabled item pickups:    " @ Round( NumDisabledItemPickups ));
        LogInternal("~~LogItemPickups: Sleeping item pickups:    " @ Round( NumSleepingItemPickups ));
        LogInternal("~~LogItemPickups: Active item pickups:      " @ Round( NumActiveItemPickups ));
        LogInternal("~~LogItemPickups: Percentages: Disabled:" @ (100 / NumTotalItemPickups) * NumDisabledItemPickups $ "%" @ "Sleeping:" @ (100 / NumTotalItemPickups) * NumSleepingItemPickups $ "%" @ "Active:" @ (100 / NumTotalItemPickups) * NumActiveItemPickups $ "%");
...
exec function LogServerMovementVars()
{
	if ( Pawn != None )
	{
		LogInternal(Pawn@"Velocity:" @ Pawn.Velocity);
		LogInternal(Pawn@"Acceleration:" @ Pawn.Acceleration);
		LogInternal(Pawn@"Physics:" @ Pawn.Physics);
		LogInternal(Pawn@"bIsWalking:" @ Pawn.bIsWalking);
		LogInternal(Pawn@"bIsSprinting:" @ KFPawn(Pawn).bIsSprinting);
		LogInternal(Pawn@"bIsCrouched:" @ Pawn.bIsCrouched);
		LogInternal(Pawn@"WeaponEncumbrance:" @ KFInventoryManager(Pawn.InvManager).GetEncumbranceSpeedMod());
		LogInternal(Pawn@"GroundSpeed:" @ Pawn.GroundSpeed);
		LogInternal(Pawn@"Speed:" @ VSize(Pawn.Velocity));
		LogInternal(Pawn@"MovementSpeedModifier:" @ Pawn.MovementSpeedModifier);
        LogInternal(Pawn@"Controller State:"@Outer.GetStateName());
	}
}
...
function OnLoginComplete(bool bWasSuccessful, string SessionTicket, string PlayfabId)
{
	...
	LogInternal("Login complete with success"@bWasSuccessful@"and playfab ID"@PlayfabId@"and session ticket"@SessionTicket);
}
...
function OnFindOnlinePlayfabGamesComplete( bool bSuccess )
{
	...
	LogInternal("Search finished with success"@bSuccess);
	...
	for( i = 0; i < SearchDataStore.GameSearchCfgList[0].Search.Results.Length; i++ )
	{
		...
		LogInternal("Listing result with region"@GS.Region@"and join string"@GS.JoinString@"and gamemode"@GS.Mode@"and lobby ID"@GS.LobbyId@"and num open connections"@GS.NumOpenPublicConnections@"and max players"@GS.NumPublicConnections);
	}
}
...
function OnQueryServerInfoComplete(bool bWasSuccessful, string LobbyId, string ServerIp, int ServerPort, string AuthTicket)
{
	...
	LogInternal("OnQueryServerInfoComplete complete with succcess"@bWasSuccessful@"and lobby ID"@LobbyId@"and server IP"@ServerIp@"and port"@ServerPort@"and auth ticket"@AuthTicket);
}
...
exec function DumpItemDefinitions()
{
	...
	{
		LogInternal("Listing item"@OSS.ItemPropertiesList[i].Definition@"with name"@OSS.ItemPropertiesList[i].Name@"and type"@OSS.ItemPropertiesList[i].Type@"and price"@OSS.ItemPropertiesList[i].Price@"and URL"@OSS.ItemPropertiesList[i].IconURL@OSS.ItemPropertiesList[i].IconURLLarge);
...
exec function DumpInventory()
{
	...
	{
		LogInternal("Listing item"@OSS.CurrentInventory[i].Definition@"and quantity"@OSS.CurrentInventory[i].Quantity);
	}
}
...
exec function DumpExchangeRules()
{
	...
	{
		LogInternal("For target"@OSS.ExchangeRuleSetList[i].Target@"and item type"@OSS.ExchangeRuleSetList[i].Type);
...
defaultproperties
{
   ...
   Name="Default__KFCheatManager"
   ObjectArchetype=GameCheatManager'GameFramework.Default__GameCheatManager'
}
