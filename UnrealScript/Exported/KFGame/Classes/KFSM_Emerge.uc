...
static function int GetEmergeAnimNum( byte EmergeType )
{
	...
	LogInternal(GetFuncName()@"Failed to find valid anim for:"$EmergeType);
}
...
function FindAnchorFailsafe()
{
	if ( PawnOwner.Role == ROLE_Authority && !PawnOwner.IsHumanControlled() )
	{
		if ( (PawnOwner.WorldInfo.TimeSeconds - PawnOwner.FindAnchorFailedTime) < 5.f)
		{
			WarnInternal("Zed unable to resume pathing after SM_Emerge"@PawnOwner@KFPOwner.SpecialMove);
			PawnOwner.Died( None, PawnOwner.WorldInfo.KillzDamageType, PawnOwner.Location );
		}
	}
}
