...
/*********************************************************************************************
 * @name    Shared Boss Functionality
 ********************************************************************************************/
/** Whether or not the game info has overriden this pawn as a boss replacement */
...
replication
{
	...
	if (bNetDirty)
		...
		RepInflateMatParam, RepDamageInflateParam, RepBleedInflateMatParam, bDisableGoreMeshWhileAlive, bOverrideAsBoss,
...
{
	if( !class'Engine'.static.GetEngine().bDIsableAILogging && MyKFAIC!= None ) { MyKFAIC.AILog_Internal(GetFuncName()$"() Wall: "$Wall,'BumpEvent'); };
...
event StuckOnPawn (Pawn OtherPawn)
{
	...
	if( MyKFAIC != none )
	{
		if( !class'Engine'.static.GetEngine().bDIsableAILogging && MyKFAIC!= None ) { MyKFAIC.AILog_Internal(self$" StuckOnPawn event at "$Location$" Base: "$Base,); };
...
function NotifyTakeHit(Controller InstigatedBy, vector HitLocation, int Damage, class<DamageType> DamageType, vector Momentum, Actor DamageCauser)
{
	...
	if( !class'Engine'.static.GetEngine().bDIsableAILogging && MyKFAIC!= None ) { MyKFAIC.AILog_Internal(GetFuncName()$"() Instigator:"$InstigatedBy$" DT: "$DamageType,'Damage'); };
}
...
simulated function bool LookAtPawn( optional Pawn P, optional float Strength=0.5f )
{
	...
	if( IK_Look_Head != none )
	{
		...
		//	{
				...
				if( !class'Engine'.static.GetEngine().bDIsableAILogging && MyKFAIC!= None ) { MyKFAIC.AILog_Internal(GetFuncName()@P,'HeadTracking'); };
...
simulated function PlayDeadHitEffects(vector HitLocation, vector HitDirection, int HitZoneIndex, name HitZoneName, name HitBoneName, class<KFDamageType> DmgType, optional bool bUseHitImpulse)
{
	...
	if( bAllowRagdollAndGoreOnDeadBodies || (WorldInfo.TimeSeconds - TimeOfDeath) <= 3.f )
	{
		...
		if(GoreManager.AllowMutilation() && HitFxInfo.bObliterated && (WorldInfo.TimeSeconds - TimeOfDeath) < 0.25f && !bUseDamageInflation)
		{
...
defaultproperties
{
   ...
   IntendedDamageInflationPercent=1.000000
   DamageInflationPercent=1.000000
   ...
   SpeedAdjustTransitionRate=200.000000
