...
simulated function SetupFX()
{
	...
	for (ChainIdx=0; ChainIdx<BoneChain.length; ++ChainIdx)
	{
		...
		{
			LogInternal("Warning!  Main chain bone"@BoneChain[ChainIdx].BoneName@"not found in FlameThrowerSpray skeletal mesh!  Bad things may happen.  Bad things.");
		}
	}
...
function SpawnImpactProjectile(vector SpawnLocation, vector SpawnRotation)
{
	...
	for( i = 0; i < MAX_SPRAY_FX_INFOS; ++i )
	{
		...
		if( (WorldInfo.TimeSeconds - SprayFxInfos[i].TimeStamp) > 10.f )
		{
			continue;
		}
