...
exec function Avatar( name ClassName )
{
	...
	{
		LogInternal("Avatar: Couldn't find any Pawn to possess of class '" $ ClassName $ "'");
	}
}
...
exec function TestLevel()
{
	...
	if ( bFoundErrors )
	{
		LogInternal("Found problem with "$Found);
		assert(false);
	}
}
...
exec function DumpOnlineSessionState()
{
	...
	if (WorldInfo.NetMode != NM_Client)
	{
		...
		LogInternal("");
		// Log game info data
		LogInternal("Class: "$WorldInfo.Game.Class.Name);
		// Log player count information
		LogInternal("  MaxPlayersAllowed: "$WorldInfo.Game.MaxPlayersAllowed);
		LogInternal("  MaxPlayers: "$WorldInfo.Game.MaxPlayers);
		LogInternal("  NumPlayers: "$WorldInfo.Game.NumPlayers);
		LogInternal("  MaxSpectatorsAllowed: "$WorldInfo.Game.MaxSpectatorsAllowed);
		LogInternal("  MaxSpectators: "$WorldInfo.Game.MaxSpectators);
		LogInternal("  NumSpectators: "$WorldInfo.Game.NumSpectators);
		LogInternal("  NumBots: "$WorldInfo.Game.NumBots);

		LogInternal("  bUseSeamlessTravel: "$WorldInfo.Game.bUseSeamlessTravel);
		LogInternal("  bRequiresPushToTalk: "$WorldInfo.Game.bRequiresPushToTalk);
		LogInternal("  bHasNetworkError: "$WorldInfo.Game.bHasNetworkError);

		LogInternal("  OnlineGameSettingsClass: "$WorldInfo.Game.OnlineGameSettingsClass);
		LogInternal("  OnlineStatsWriteClass: "$WorldInfo.Game.OnlineStatsWriteClass);

		LogInternal("  bUsingArbitration: "$WorldInfo.Game.bUsingArbitration);
		if (WorldInfo.Game.bUsingArbitration)
		{
			LogInternal("  bHasArbitratedHandshakeBegun: "$WorldInfo.Game.bHasArbitratedHandshakeBegun);
			LogInternal("  bNeedsEndGameHandshake: "$WorldInfo.Game.bNeedsEndGameHandshake);
			LogInternal("  bIsEndGameHandshakeComplete: "$WorldInfo.Game.bIsEndGameHandshakeComplete);
			LogInternal("  bHasEndGameHandshakeBegun: "$WorldInfo.Game.bHasEndGameHandshakeBegun);
			LogInternal("  ArbitrationHandshakeTimeout: "$WorldInfo.Game.ArbitrationHandshakeTimeout);
			LogInternal("  Number of pending arbitration PCs: "$WorldInfo.Game.PendingArbitrationPCs.Length);
			...
			{
				LogInternal("    Player: "$WorldInfo.Game.PendingArbitrationPCs[PlayerIndex].PlayerReplicationInfo.PlayerName$" PC ("$WorldInfo.Game.PendingArbitrationPCs[PlayerIndex].Name$")");
			}
			LogInternal("  Number of arbitration PCs: "$WorldInfo.Game.ArbitrationPCs.Length);
			...
			{
				LogInternal("    Player: "$WorldInfo.Game.ArbitrationPCs[PlayerIndex].PlayerReplicationInfo.PlayerName$" PC ("$WorldInfo.Game.ArbitrationPCs[PlayerIndex].Name$")");
			}
		}
	}
	// Log PRI player info
...
exec function DebugDownloadTitleFile(string Filename, optional bool bFromCache)
{
	if (OnlineSub != None)
	{
		if (bFromCache)
		{
			...
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "starting file load for"@Filename);
			...
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "OnlineTitleFileCacheInterface not supported");
			}
		}
		else
		{
			...
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "starting file download request for"@Filename);
			...
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "OnlineTitleFileInterface not supported");
			}
		}
	}
}
function OnDownloadComplete(bool bWasSuccessful,string Filename)
{
	...
	LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "download completed"
...
function OnLoadComplete(bool bWasSuccessful,string FileName)
{
	...
	LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "load completed"
		@"bWasSuccessful="$bWasSuccessful
		@"FileName="$Filename);
...
exec function DebugSaveTitleFile(string Filename)
{
	...
	if (OnlineSub != None)
	{
		if (OnlineSub.TitleFileInterface != None)
		{
			...
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "found file in download cache. using file contents from download cache:"@Filename);						
			}
			else
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "couldn't find file in download cache:"@Filename);						
			}
		}
		else
		{
		...
		if (OnlineSub.TitleFileCacheInterface != None)
		{
			LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "starting file save for"@Filename);
...
function OnSaveComplete(bool bWasSuccessful,string FileName)
{
	...
	LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "save completed"
		@"bWasSuccessful="$bWasSuccessful
		@"FileName="$Filename);
}
...
exec function DebugDeleteTitleFiles()
{
	if (OnlineSub != None)
	{
		if (OnlineSub.TitleFileCacheInterface != None)
		{
			LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "deleting all title files in cache dir");
			if (OnlineSub.TitleFileCacheInterface.DeleteTitleFiles(0))
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "delete succeeded");
			}
			else
			{
				LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@ "cant delete. file ops in progress");
			}		
		}
		else
		{
...
exec function DebugNotification(string MessageBody, int SecondsFromNow)
{
	...
	if (AppNotification != None)
	{
		...
		LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@""
...
{
	LogInternal("("$Name$") CheatManager::"$GetStateName()$":"$GetFuncName()@"bWasAppActive="$bWasAppActive);
...
exec function TestHttp(string Verb, string Payload, string URL, optional bool bSendParallelRequest)
{
	...
	LogInternal("ContentLength ="@R.GetContentLength());
...
function OnRequestComplete(HttpRequestInterface OriginalRequest, HttpResponseInterface Response, bool bDidSucceed)
{
	...
	LogInternal("Got response!!!!!!! Succeeded="@bDidSucceed);
	...
	if (Response != None)
	{
		...
		LogInternal("Response Code="@Response.GetResponseCode());
