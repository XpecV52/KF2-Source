...
function InitializeOptions(string Options)
{
	...
	if( InOpt != "" )
	{
		LogInternal("bUsingAutomatedTestingMapList: "$bool(InOpt));
	...
	if( InOpt != "" )
	{
		LogInternal("bAutomatedTestingWithOpen: "$bool(InOpt));
	...
	if (InOpt != "")
	{
		LogInternal("AutomatedTestingMapIndex: " $ int(InOpt));
	...
	if ( bAutomatedTestingWithOpen )
	{
		...
		if (InOpt != "")
		{
			LogInternal("NumberOfMatchesPlayed: " $ int(InOpt));
		...
		if (InOpt != "")
		{
			LogInternal("NumMapListCyclesDone: " $ int(InOpt));
...
state TravelTheWorld
{
	...
	function PrintOutTravelWorldTimes( const int TotalTimeInSeconds )
	{
		...
		LogInternal(WorldInfo.GetMapName() $ ": Traveling this map will take approx TotalSeconds: " $ TotalTimeInSeconds $ "   Hours: " $ Hours $ "  Minutes: " $ Minutes $ "  Seconds: " $ Seconds);
	...
	function SetIncrementsForLoops( const float NumTravelLocations )
	{
		...
		if( CalcTravelTheWorldTime( NumTravelLocations, 8 ) < TimeWeGetInSeconds )
		{
			...
			LogInternal(WorldInfo.GetMapName() $ " SetIncrementsForLoops: TravelPointsIncrement: " $ TravelPointsIncrement $ " NumRotationsIncrement: " $ NumRotationsIncrement $ " for NumTravelLocations: " $ NumTravelLocations);
...
Begin:
	...
	{
		LogInternal("StreamLevelOut: " $ Worldinfo.StreamingLevels[SentinelIdx].PackageName);
	...
	for( SentinelIdx = 0; SentinelIdx < WorldInfo.StreamingLevels.length; ++SentinelIdx )
	{
		LogInternal("Gathering locations for: " $ Worldinfo.StreamingLevels[SentinelIdx].PackageName);
	...
	LogInternal(WorldInfo.GetMapName() $ " COMPLETED LEVEL INTEROGATION!! Total TravelPoints: " $ SentinelTravelArray.Length);
	...
	for( SentinelIdx = 0; SentinelIdx < WorldInfo.StreamingLevels.length; ++SentinelIdx )
	{
		...
		{
			LogInternal("   Found a LevelStreamingAlwaysLoaded" @ Worldinfo.StreamingLevels[SentinelIdx].PackageName);
			SentinelPC.ClientUpdateLevelStreamingStatus( Worldinfo.StreamingLevels[SentinelIdx].PackageName, TRUE, TRUE, TRUE );
		}
	}
	...
	LogInternal("   SentinelTravelArray.length " $ SentinelTravelArray.length);
	for( SentinelNavigationIdx = 0; SentinelNavigationIdx < SentinelTravelArray.length; SentinelNavigationIdx+=TravelPointsIncrement )
	{
		LogInternal("Going to:" @ SentinelTravelArray[SentinelNavigationIdx] @ SentinelNavigationIdx $ " of " $ SentinelTravelArray.length);
...
function IncrementAutomatedTestingMapIndex()
{
	if( bUsingAutomatedTestingMapList == TRUE )
	{
		...
		{
			LogInternal("  NumMapListCyclesDone: " $ NumMapListCyclesDone $ " / " $ NumAutomatedMapTestingCycles);
		}
		else
		{
			if (AutomatedTestingMapIndex >= 0)
		...
		LogInternal("  NextIncrementAutomatedTestingMapIndex: " $ AutomatedTestingMapIndex $ " / " $ AutomatedMapTestingList.Length);
	}
}

function IncrementNumberOfMatchesPlayed()
{
	LogInternal("  Num Matches Played: " $ NumberOfMatchesPlayed);
