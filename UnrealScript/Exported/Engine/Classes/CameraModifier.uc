...
function bool AddCameraModifier( Camera Camera )
{
	...
	for( ModifierIdx = 0; ModifierIdx < Camera.ModifierList.Length; ModifierIdx++ )
	{
		...
		{
			LogInternal("AddCameraModifier found existing modifier in list, replacing with new one" @ self);
	...
	if( bDebug )
	{
		LogInternal("AddModifier"@BestIdx@self);
		for( ModifierIdx = 0; ModifierIdx < Camera.ModifierList.Length; ModifierIdx++ )
		{
			LogInternal(Camera.ModifierList[ModifierIdx]@"Idx"@ModifierIdx@"Pri"@Camera.ModifierList[ModifierIdx].Priority);
		}
		LogInternal("****************");
	}
...
{
	//debug
	if (bDebug) LogInternal(self@"DisableModifier"@bImmediate);
