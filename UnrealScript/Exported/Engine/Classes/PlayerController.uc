...
function OnControllerChanged(int ControllerId,bool bIsConnected,bool bPauseGame)
{
	...
	&&	(WorldInfo.Game == None || !WorldInfo.Game.IsAutomatedPerfTesting()))
	{
		...
		LogInternal("Received gamepad connection change for player" @ class'UIInteraction'.static.GetPlayerIndex(ControllerId) $ ": gamepad" @ ControllerId @ "is now" @ (bIsConnected ? "connected" : "disconnected"));
...
event NotifyPeerDisconnectHost(UniqueNetId PeerNetId)
{
	...
	LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ ": client peer lost connection to host"
...
event bool MigrateNewHost()
{
	...
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "migrating player as host"
...
function OnHostMigratedOnlineGame(name SessionName,bool bWasSuccessful)
{
	...
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "migration failed for"
			$" NetId="$class'OnlineSubsystem'.static.UniqueNetIdToString(PlayerReplicationInfo.UniqueId),'DevNet');
...
event PeerReceivedMigratedSession(UniqueNetId FromPeerNetId, name SessionName, class<OnlineGameSearch> SearchClass, byte PlatformSpecificInfo[80])
{
	...
	if (LP != None &&
		...
		OnlineSub.GameInterface != None)
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "received migrated session to join"
...
,'DevNet');
		...
		{
			LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "failed to bind to migrated session!",'DevNet');
			MigratedSearchToJoin = None;
...
function PeerDesignatedAsClient(name SessionName)
{
	...
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "traveling to joined,migrated session "
			$" SessionName="$SessionName
			$" URL="$URL
,'DevNet');
	...
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "failed joining migrated session "
				$" SessionName="$SessionName
,'DevNet');
...
{
	LogInternal(">> PlayerController::ClientInitializeDataStores for player" @ Self,'DevDataStore');
...
simulated protected function RegisterCustomPlayerDataStores()
{
	...
	LogInternal(">>" @ "("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "(" $ PlayerName $ ")" @ "LP:"$(LP!= None ? string(LP.Name) : "None"),'DevDataStore');
	if ( LP != None )
	{
			...
			if ( OnlinePlayerData == None )
			{
				...
				if ( PlayerDataStoreClass != None )
				{
					...
					if ( OnlinePlayerData != None )
					{
						...
						{
							LogInternal("Failed to register 'OnlinePlayerData' data store for player:"@ Self @ "(" $ PlayerName $ ")" @ "OnlinePlayerData:"$(OnlinePlayerData!= None ? string(OnlinePlayerData.Name) : "None"),'DevDataStore');
						}
					}
					else
					{
						LogInternal("Failed to create 'OnlinePlayerData' data store for player:"@ Self @ "(" $ PlayerName $ ") using class" @ PlayerDataStoreClass,'DevDataStore');
					}
				}
				else
				{
					LogInternal("Failed to find valid data store class while attempting to register the 'OnlinePlayerData' data store for player:"@ Self @ "(" $ PlayerName $ ")",'DevDataStore');
				}
			}
			else
			{
				LogInternal("'OnlinePlayerData' data store already registered for player:"@ Self @ "(" $ PlayerName $ ")",'DevDataStore');
			}
		}

	}

	LogInternal("<<" @ "("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "(" $ PlayerName $ ")",'DevDataStore');
}
		...
		if ( DataStoreManager != None )
		{
			...
			for ( ClassIndex = 0; ClassIndex < PlayerDataStoreClasses.Length; ClassIndex++ )
			{
				...
				if ( PlayerDataStoreClass != None )
				{
					...
					{
						LogInternal("    Registering standard player data store '" $ PlayerDataStoreClass.Name $ "' for player" @ Self @ "(" $ PlayerName $ ")" @ "LP:"$(LP!= None ? string(LP.Name) : "None"),'DevDataStore');
						...
						if ( PlayerDataStore != None )
						{
							...
							{
								LogInternal("Failed to register '" $ PlayerDataStoreClass.default.Tag $ "' data store for player:" @ Self @ "(" $ PlayerName $ ")" @ "PlayerDataStore:"$(PlayerDataStore!= None ? string(PlayerDataStore.Name) : "None"),'DevDataStore');
							}
						}
					}
					else
					{
						LogInternal("'" $ PlayerDataStoreClass.default.Tag $ "' data store already registered for player:" @ Self @ "(" $ PlayerName $ ")",'DevDataStore');
					}
				}
			}
		}
	...
	if ( LP != None )
	{
		...
		if ( DataStoreManager != None )
		{
			...
			if ( OnlinePlayerDataStore != None )
			{
				...
				{
					LogInternal("Failed to unregister 'OnlinePlayerData' data store for player:"@ Self @ "(" $ PlayerName $ ")" @ "OnlinePlayerDataStore:"$(OnlinePlayerDataStore!= None ? string(OnlinePlayerDataStore.Name) : "None"),'DevDataStore');
				}
			}
			else
			{
				LogInternal("'OnlinePlayerData' data store not registered for player:" @ Self @ "(" $ PlayerName $ ")",'DevDataStore');
			}
			...
			for ( ClassIndex = 0; ClassIndex < PlayerDataStoreClasses.Length; ClassIndex++ )
			{
				...
				if ( PlayerDataStoreClass != None )
				{
					...
					if ( PlayerDataStore != None )
					{
						...
						{
							LogInternal("Failed to unregister '" $ PlayerDataStore.Tag $ "' data store for player:"@ Self @ "(" $ PlayerName $ ")" @ "PlayerDataStore:"$(PlayerDataStore!= None ? string(PlayerDataStore.Name) : "None"),'DevDataStore');
						}
					}
				}
			}
...
event ClearOnlineDelegates()
{
	...
	LogInternal("Clearing online delegates for" @ Self @ "(" $ "Player:"$(Player!= None ? string(Player.Name) : "None") $ ")",'DevOnline');
...
reliable server function ServerCamera( name NewMode )
{
	...
	if ( PlayerCamera != None )
		LogInternal("#### " $ PlayerCamera.CameraStyle);
...
)
{
	...
	else
	{
		...
		{
			LogInternal("- state change:"@GetStateName()@"->"@newstate,'PlayerMove');
	...
	if (MoveActor.Base != NewBase) LogInternal("- base mismatch:"@MoveActor.Base@NewBase,'PlayerMove');
	if (MoveActor.Location != NewLocation) LogInternal("- location mismatch, delta:"@VSize(MoveActor.Location - NewLocation),'PlayerMove');
	if (MoveActor.Velocity != NewVelocity) LogInternal("- velocity mismatch, delta:"@VSize(NewVelocity - MoveActor.Velocity)@"client:"@VSize(MoveActor.Velocity)@"server:"@VSize(NewVelocity),'PlayerMove');
...
// 	`log("Client "$Role$" adjust "$self$" stamp "$TimeStamp$" location "$MoveActor.Location);
	...
	if( bDebugClientAdjustPosition )
	{
		...
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@"!!!!!!!!!!!!!!"@SavedMoves@"Pawn.Rotation:'"$Pawn.Rotation$"'"@"WorldInfo.TimeSeconds:'"$WorldInfo.TimeSeconds$"'");
	}
...
function ClientUpdatePosition()
{
	...
	if( bDebugClientAdjustPosition )
	{
		...
		LogInternal("Old"@Pawn.Location@Pawn.bRootMotionFromInterpCurve@Pawn.RootMotionInterpCurrentTime);
	...
	if( bDebugClientAdjustPosition )
	{
		LogInternal("New"@Pawn.Location@Pawn.bRootMotionFromInterpCurve@Pawn.RootMotionInterpCurrentTime);
...
exec function UTrace()
{
	...
	LogInternal("UTracing changed to "$ IsUTracing() $ " at "$ WorldInfo.TimeSeconds,'UTrace');
}
...
reliable client event ClientSetProgressMessage(EProgressMessageType MessageType, string Message, optional string Title, optional bool bIgnoreFutureNetworkMessages)
{
	...
	{
		WarnInternal("Discarded progress message due to no viewport:" @ MessageType @ Message @ Title);
	}
}
...
state Spectating extends BaseSpectating
{
	...
	event EndState(Name NextStateName)
	{
		if ( PlayerReplicationInfo != None )
		{
			if ( PlayerReplicationInfo.bOnlySpectator )
			{
				LogInternal("WARNING - Spectator only player leaving spectating state to go to "$NextStateName);
			}
...
{
	LogInternal("Dumping state stack for" @ Self);
	DumpStateStack();
}
...
exec function ShowGameState()
{
	if ( WorldInfo.Game != None )
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()$": Dumping state stack for" @ WorldInfo.Game);
		WorldInfo.Game.DumpStateStack();
	}
	else
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()$": No GameInfo found!");
	}
}
...
exec function SaveClassConfig(coerce string className)
{
	...
	if (saveClass != None)
	{
		LogInternal("- Saving config on:"@saveClass);
...
exec function SaveActorConfig(coerce Name actorName)
{
	...
	LogInternal("SaveActorConfig:"@actorName);
	foreach AllActors(class'Actor',chkActor)
	{
		if (chkActor != None &&
			chkActor.Name == actorName)
		{
			LogInternal("- Saving config on:"@chkActor);
			chkActor.SaveConfig();
		}
	}
}
...
{
	LogInternal("SESSIONS - Destroy for invite complete for session name"@SessionName@"and successful"@bWasSuccessful);
...
{
	LogInternal("  Num Matches Played: " $ PlayerReplicationInfo.AutomatedTestingData.NumberOfMatchesPlayed);
...
reliable client function ClientTravelToSession(name SessionName,class<OnlineGameSearch> SearchClass,byte PlatformSpecificInfo[80])
{
	...
	if (LP != None)
	{
		...
		if (OnlineSub.GameInterface.BindPlatformSpecificSessionToSearch(LP.ControllerId,Search,PlatformSpecificInfo))
		{
			...
			LogInternal("(PlayerController.ClientTravelToSession): "
				$" SessionName="$SessionName
				$" SearchClass="$SearchClass
				$" UniqueId="$OnlineSub.static.UniqueNetIdToString(PlayerReplicationInfo.UniqueId)
...
reliable client function ClientNotifyPartyHostLeaving(UniqueNetId PartyHostPlayerId)
{
	...
	{
		//@todo sz - implement
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "Party host is leaving: "
	...
	if ( Player != None )
	{
		if ( IsSplitscreenPlayer() )
		{
			...
			if ( LP != None )
			{
				...
				{
					WarnInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()$ ":" @ "requested player at invalid index!" @ "PlayerIndex:'"$PlayerIndex$"'" @ "NumLocalPlayers:'"$LP.ViewportClient.Outer.GamePlayers.Length$"'");
				}
			}
			else if ( RemoteConnection != None )
			{
				if ( WorldInfo.NetMode == NM_Client )
				{
					...
					WarnInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()$ ":" @ "CALLED ON CLIENT WITH VALID REMOTE NETCONNECTION!");
				}
				else
				{
					ChildRemoteConnection = ChildConnection(RemoteConnection);
					...
					{
						LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()$ ":" @ Player @ "IS NOT THE PRIMARY CONNECTION AND HAS NO CHILD CONNECTIONS!");
					}
				}
			}
			else
			{
				LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()$ ":" @ Player @ "IS NOT A LOCALPLAYER AND NOT A REMOTECONNECTION! (No valid Player reference)");
			}
		}
	}
	else
	{
		LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()$ ":" @ "NULL value for Player!");
	}
...
simulated function int GetSplitscreenPlayerCount()
{
	...
	if ( IsSplitscreenPlayer() )
	{
		if ( Player != None )
		{
			...
			{
				LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "NOT A LOCALPLAYER AND NOT A REMOTECONNECTION!");
			}
		}
		else
		{
			LogInternal("("$Name$") PlayerController::"$GetStateName()$":"$GetFuncName()@ "called without a valid Player value!");
		}
	}
...
function OnCameraShake(SeqAct_CameraShake inAction)
{
	if (inAction.InputLinks[0].bHasImpulse)
	{
		...
		if (InAction.bRadialShake)
		{
			...
			{
				WarnInternal(self@"Location actor needed for bRadialFalloff camera shake.");
				return;
			}
		}
		else
...
{
	LogInternal("BugItGo to:" @ TheLocation @ TheRotation);
 ...
 // Just avoiding an accessed none if in debugcameracontroller mode where you have no pawn
	if( Pawn != none )
	{
...
function DebugLogPRIs()
{
	...
	if (WorldInfo != None &&
		WorldInfo.GRI != None)
	{
		LogInternal("  Number of PRI players: "$WorldInfo.GRI.PRIArray.Length);
		...
		for (PlayerIndex = 0; PlayerIndex < WorldInfo.GRI.PRIArray.Length; PlayerIndex++)
		{
			...
			LogInternal("    Player: "$WorldInfo.GRI.PRIArray[PlayerIndex].PlayerName$" UID ("$class'OnlineSubsystem'.static.UniqueNetIdToString(NetId)$") PC ("$WorldInfo.GRI.PRIArray[PlayerIndex].Owner$")");
		}
		LogInternal("");
	}
}
	...
	foreach WorldInfo.AllControllers(class'PlayerController', PC)
	{
		if (WorldInfo.NetMode == NM_Client || !PC.IsLocalPlayerController())
		{
			...
			for (PeerIdx = 0; PeerIdx < PC.ConnectedPeers.Length; PeerIdx++)
			{
				...
				LogInternal("      "$class'OnlineSubsystem'.static.UniqueNetIdToString(PeerInfo.PlayerId)
