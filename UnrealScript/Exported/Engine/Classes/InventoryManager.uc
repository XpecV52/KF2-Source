...
simulated function bool AddInventory(Inventory NewItem, optional bool bDoNotActivate)
{
	...
	if( (NewItem != None) && !NewItem.bDeleteMe )
	{
		...
		LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "adding" @ NewItem @ "bDoNotActivate:" @ bDoNotActivate,'Inventory');
...
simulated function RemoveFromInventory(Inventory ItemToRemove)
{
	...
	if( ItemToRemove != None )
	{
		...
		if (Instigator.Health > 0 && Instigator.Weapon == None)
		{
			...
			{
				LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "Calling ClientSwitchToBestWeapon to make sure a weapon is brought up",'Inventory');
...
simulated event DiscardInventory()
{
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "",'Inventory');
...
simulated private function InternalSetCurrentWeapon(Weapon DesiredWeapon)
{
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "PrevWeapon:" @ PrevWeapon @ "DesiredWeapon:" @ DesiredWeapon,'Inventory');
	...
	if( PrevWeapon != None && DesiredWeapon == PrevWeapon && !PrevWeapon.IsInState('WeaponPuttingDown') )
	{
		...
		{
			LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "DesiredWeapon == PrevWeapon - abort"@DesiredWeapon.GetStateName(),'Inventory');
			return;
		}
	}
	...
	if( PrevWeapon != None && PrevWeapon != DesiredWeapon && !PrevWeapon.bDeleteMe && !PrevWeapon.IsInState('Inactive') )
	{
		...
		LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "Try to put down previous weapon first.",'Inventory');
...
{
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "SetPendingWeapon to" @ DesiredWeapon,'Inventory');
	// set the new weapon as pending
...
{
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "PendingWeapon:'"$PendingWeapon$"'",'Inventory');
...
simulated function ChangedWeapon()
{
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "PendingWeapon:'"$PendingWeapon$"'"@"bMustHoldWeapon:'"$bMustHoldWeapon$"'",'Inventory');
	if( PendingWeapon == None && bMustHoldWeapon )
	{
		if( OldWeapon != None )
		{
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "switch from" @ OldWeapon @ "to" @ PendingWeapon,'Inventory');
...
simulated function ClientWeaponSet(Weapon NewWeapon, bool bOptionalSet, optional bool bDoNotActivate)
{
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "NewWeapon:" @ NewWeapon @ "bOptionalSet:" @ bOptionalSet @ "bDoNotActivate:" @ bDoNotActivate,'Inventory');
	if( !bDoNotActivate )
	{
		OldWeapon = Instigator.Weapon;
		...
		{
			LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "OldWeapon == None or Inactive - Set new weapon right away" @ NewWeapon,'Inventory');
		...
		if( OldWeapon == NewWeapon )
		{
			...
			{
				LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "OldWeapon == NewWeapon - but in PendingClientWeaponSet, so reset." @ NewWeapon,'Inventory');
				SetCurrentWeapon(NewWeapon);
			}
			else
			{
				LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "OldWeapon == NewWeapon - abort" @ NewWeapon,'Inventory');
			}
			return;
		}
		...
		if( bOptionalSet )
		{
			...
			{
				LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "bOptionalSet && (DenyClientWeaponSet() || bNeverSwitchOnPickup) - abort" @ NewWeapon,'Inventory');
		...
		if( PendingWeapon == None || !PendingWeapon.HasAnyAmmo() || PendingWeapon.GetWeaponRating() < NewWeapon.GetWeaponRating() )
		{
			...
			{
				LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "Switch to new weapon:" @ NewWeapon,'Inventory');
				SetCurrentWeapon(NewWeapon);
				return;
			}
		}
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "Send to inactive state" @ NewWeapon,'Inventory');
