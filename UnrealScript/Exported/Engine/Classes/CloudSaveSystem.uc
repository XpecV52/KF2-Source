...
final function OnDeleteSaveDataComplete(bool bWasSucessfull, string StorageID, string BlobName, string Error)
{
	...
	if (Index != INDEX_NONE)
	{
		...
		{
			WarnInternal("CloudSaveSystem in corrupt stat DeleteSaveData request finished but active slot operation should have been"@ActiveSaveSlotOperations[Index].SlotOperation);
		}
		ActiveSaveSlotOperations.Remove(Index, 1);
	}
	else
