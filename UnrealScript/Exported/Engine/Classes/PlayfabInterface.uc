...
function AddGetPlayerListCompleteDelegate(delegate<OnGetPlayerListComplete> InDelegate) { if (GetPlayerListCompleteDelegates.Find(InDelegate) == INDEX_NONE){GetPlayerListCompleteDelegates[GetPlayerListCompleteDelegates.Length] = InDelegate;}; }
function ClearGetPlayerListCompleteDelegate(delegate<OnGetPlayerListComplete> InDelegate) { local int RemoveIndex;RemoveIndex = GetPlayerListCompleteDelegates.Find(InDelegate);if (RemoveIndex != INDEX_NONE){GetPlayerListCompleteDelegates.Remove(RemoveIndex,1);}; }
