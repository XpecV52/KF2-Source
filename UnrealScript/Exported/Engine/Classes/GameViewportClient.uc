...
event LocalPlayer CreatePlayer(int ControllerId, out string OutError, bool bSpawnActor)
{
	...
	LogInternal("Creating new player with ControllerId" @ ControllerId @ "(" $ GamePlayers.Length @ "existing players)",'PlayerManagement');
	...
	{
		LogInternal("Successfully created new player with ControllerId" @ ControllerId $ ":" @ NewPlayer @ "- inserted into GamePlayers array at index" @ InsertIndex
...
event bool RemovePlayer(LocalPlayer ExPlayer)
{
	...
	{
		LogInternal("Removing player" @ ExPlayer @ " with ControllerId" @ ExPlayer.ControllerId @ "at index" @ GamePlayers.Find(ExPlayer)@ "(" $ GamePlayers.Length @ "existing players)",'PlayerManagement');
		...
		LogInternal("Finished removing player " @ ExPlayer @ " with ControllerId" @ ExPlayer.ControllerId @ "at index" @ OldIndex@ "(" $ GamePlayers.Length @ "remaining players)",'PlayerManagement');
		return true;
	}
	else
	{
		LogInternal("Not removing player" @ ExPlayer @ " with ControllerId" @ ExPlayer.ControllerId @ "because player does not have appropriate role (" $ GetEnum(enum'ENetRole',ExPlayer.Actor.Role) $ ")",'PlayerManagement');
		return false;
	}
}
...
event int InsertInteraction( Interaction NewInteraction, optional int InIndex = -1 )
{
	...
	if ( NewInteraction != None )
	{
		...
		{
			WarnInternal("Invalid insertion index specified:" @ InIndex);
		}
	}
...
event SetProgressMessage(EProgressMessageType MessageType, string Message, optional string Title, optional bool bIgnoreFutureNetworkMessages)
{
	...
	else
	{
		if (MessageType == PMT_ConnectionFailure || MessageType == PMT_SocketFailure)
		{
			...
			{
				LogInternal("("$Name$") GameViewportClient::"$GetStateName()$":"$GetFuncName()@ "MessageType="$MessageType @ "Message="$Message @ ": host migration started.. "$WI.PeerHostMigration.HostMigrationProgress,'DevNet');
			}
			//@FIXME: bIgnoreNetworkMessages needs to die
			else if (!Outer.GamePlayers[0].Actor.bIgnoreNetworkMessages)
			{
				LogInternal("("$Name$") GameViewportClient::"$GetStateName()$":"$GetFuncName()@ "MessageType="$MessageType @ "Message="$Message @ ": host migration not enabled.. handling connection error.",'DevNet');
...
function NotifyConnectionError(EProgressMessageType MessageType, optional string Message=Localize("Errors", "ConnectionFailed", "Engine"), optional string Title=Localize("Errors", "ConnectionFailed_Title", "Engine") )
{
	...
	WI = class'Engine'.static.GetCurrentWorldInfo();
	LogInternal("("$Name$") GameViewportClient::"$GetStateName()$":"$GetFuncName()@ "Title:'"$Title$"'" @ "Message:'"$Message$"'" @ "NetMode:'"$GetEnum(Enum'ENetMode',WI.NetMode)$"'" @ "Map:'"$WI.GetURLMap()$"'",'DevNet');
...
function BecomePrimaryPlayer(int PlayerIndex)
{
	...
	if (UIController != None && PlayerIndex > 0 && PlayerIndex < UIController.GetPlayerCount())
	{
		...
		if (PlayerOwner == None)
		{
			LogInternal("GameViewportClient:BecomePrimaryPlayer has failed to find the player owner for index" @ PlayerIndex @ "ABORTING!!!");
			return;
		}
...
defaultproperties
{
   ...
   ObjectArchetype=ScriptViewportClient'Engine.Default__ScriptViewportClient'
}
