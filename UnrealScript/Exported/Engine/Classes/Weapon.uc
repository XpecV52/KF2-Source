...
{
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "bOptionalSet:" @ bOptionalSet @ "bDoNotActivate:" @ bDoNotActivate @ "Instigator:" @ Instigator @ "InvManager:" @ InvManager,'Inventory');
	...
	if( Instigator == None )
	{
		LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "Instigator == None, going to PendingClientWeaponSet",'Inventory');
	...
	if( InvManager == None )
	{
		LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "InvManager == None, going to PendingClientWeaponSet",'Inventory');
		GotoState('PendingClientWeaponSet');
		return;
	}
...
simulated function BeginFire(Byte FireModeNum)
{
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "FireModeNum:" @ FireModeNum,'Inventory');
...
simulated function SendToFiringState(byte FireModeNum)
{
	...
	{
		LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "Invalid FireModeNum",'Inventory');
		return;
	}
	...
	LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ FireModeNum @ "Sending to state:" @ FiringStatesArray[FireModeNum],'Inventory');
...
//		ForceEndFire();
	...
	reliable server function ServerStartFire(byte FireModeNum)
	{
		...
		WarnInternal(WorldInfo.TimeSeconds @ Instigator @ "received ServerStartFire in Inactive State!!!");
...
simulated state Active
{
	...
	simulated event BeginState(Name PreviousStateName)
	{
		...
		// Check to see if we need to go down
   		if( bWeaponPutDown )
		{
			LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "Weapon put down requested during transition, put it down now",'Inventory');
			PutDownWeapon();
		}
		else if ( !HasAnyAmmo() )
		{
...
simulated state WeaponFiring
{
	...
	{
		LogInternal(WorldInfo.TimeSeconds @ "Self:" @ Self @ "Instigator:" @ Instigator @ GetStateName() $ "::" $ GetFuncName() @ "NextStateName:" @ NextStateName,'Inventory');
...
simulated function float GetTargetDistance( )
{
  ...
  // Custom lighting channel implementation
