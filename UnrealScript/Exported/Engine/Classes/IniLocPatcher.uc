...
function OnRequestTitleFileListComplete(bool bWasSuccessful, string ResultStr)
{
	...
	if (bWasSuccessful)
	{
		...
		{
			LogInternal("("$Name$") IniLocPatcher::"$GetStateName()$":"$GetFuncName()@"Download of file list failed. Bad json."
				@"ResultStr="$ResultStr);
	...
	{
		LogInternal("("$Name$") IniLocPatcher::"$GetStateName()$":"$GetFuncName()@"Download of file list failed.",'DevConfig');
...
function StartLoadingFiles()
{
	...
	{
		LogInternal("("$Name$") IniLocPatcher::"$GetStateName()$":"$GetFuncName()@"if (bRequestEmsFileList && TitleFileCacheInterface != None)"@"Files.Length:'"$Files.Length$"'",'DevConfig');
	...
	{
		LogInternal("("$Name$") IniLocPatcher::"$GetStateName()$":"$GetFuncName()@"else if (TitleFileInterface != None)"@"Files.Length:'"$Files.Length$"'",'DevConfig');
		// Iterate through files trying to download them
		for (Index = 0; Index < Files.Length; Index++)
		{
			// Kick off the read of that file if not already started or failed
...
function OnDownloadFileComplete(bool bWasSuccessful,string FileName)
{
	...
	for (Index = 0; Index < Files.Length; Index++)
	{
		if (Files[Index].DLName == FileName)
		{
			...
			{
				LogInternal("Failed to download the file from system interface."
	...
	for (Index = 0; Index < Files.Length; Index++)
	{
		if (Files[Index].DLName == FileName)
		{
			if (bWasSuccessful)
			{
				...
				{
					LogInternal("Hash for file cache entry not valid."
						@"DLName="$Files[Index].DLName
						@"Filename="$Files[Index].Filename);
				}
			}
