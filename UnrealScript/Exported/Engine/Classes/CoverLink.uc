...
simulated final event bool UnClaim( Pawn OldClaim, int SlotIdx, bool bUnclaimAll )
{
	...
	if( bDebug )
	{
		LogInternal(self@"UnClaim"@"OldClaim:'"$OldClaim$"'"@"SlotIdx:'"$SlotIdx$"'"@"bUnclaimAll:'"$bUnclaimAll$"'"@"bClaimAllSlots:'"$bClaimAllSlots$"'");
	}
	...
	if( bDebug )
	{
		...
		LogInternal(self@"Claims from"@"OldClaim:'"$OldClaim$"'"@"NumClaims:'"$NumClaims$"'"@"Slots:"@Str);
...
defaultproperties
{
   bAutoSort=True
   bAutoAdjust=True
   LeanTraceDist=64.000000
   Slots(0)=(LocationOffset=(X=64.000000,Y=0.000000,Z=0.000000))
   InvalidateDistance=64.000000
   MaxFireLinkDist=2048.000000
   AlignDist=36.000000
   AutoCoverSlotInterval=175.000000
   StandHeight=160.000000
   MidHeight=70.000000
