...
event ClearPause()
{
	...
	{
		LogInternal("Clearing list of UnPause delegates for" @ Name @ "because game type is not pauseable");
		Pausers.Length = 0;
	}
...
function DebugPause()
{
	...
	for (Index = 0; Index < Pausers.Length; Index++)
	{
		...
		if (CanUnpauseCriteriaMet())
		{
			LogInternal("Pauser in index "$Index$" thinks it's ok to unpause:" @ CanUnpauseCriteriaMet);
		}
		else
		{
			LogInternal("Pauser in index "$Index$" thinks the game should remain paused:" @ CanUnpauseCriteriaMet);
		}
	}
}
...
function UpdateNetSpeeds()
{
	...
	LogInternal("New Dynamic NetSpeed "$NewNetSpeed$" vs old "$AdjustedNetSpeed,'DevNet');
