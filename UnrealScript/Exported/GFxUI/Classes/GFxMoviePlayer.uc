...
// Refreshes the flags (gamma,etc) on the Gfx viewport.
...
function PlaySoundFromTheme(name EventName, optional name SoundThemeName='default')
{
	...
	LogInternal("gfcProcessSound Callback - Sound Theme: "$SoundThemeName$" SoundEvent: "$EventName,'DevGFxUI');
	...
	if( ThemeIndex != INDEX_NONE )
	{
		...
		{
			LogInternal("Invalid theme binding for theme " $ SoundThemeName,'GearsUI');
		}
	}
	else
	{
		LogInternal("Unable to find sound theme for movie " $ SoundThemeName,'GearsUI');
	}
}
