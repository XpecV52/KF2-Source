...
{
    if (RemoteTalkerStatusChangeDelegates.Find(TalkerDelegate) == INDEX_NONE) { RemoteTalkerStatusChangeDelegates.AddItem(TalkerDelegate);	};
}
...
{
    if (AccountPickerCompleteDelegates.Find(InDelegate) == INDEX_NONE) { AccountPickerCompleteDelegates.AddItem(InDelegate);	};
}
...
{
	if (OnLoginCompleteDelegates.Find(InDelegate) == INDEX_NONE) { OnLoginCompleteDelegates.AddItem(InDelegate);	};
}
...
{
	if (LoginFailedDelegates.Find(FailedDelegate) == INDEX_NONE) { LoginFailedDelegates.AddItem(FailedDelegate);	};
}
...
{
	if (UserControllerPairingChangeDelegates.Find(PairingChangeDelegate) == INDEX_NONE) { UserControllerPairingChangeDelegates.AddItem(PairingChangeDelegate);	};
}
...
{
    if (UserAccountInfoRetrievedDelegates.Find(InDelegate) == INDEX_NONE) { UserAccountInfoRetrievedDelegates.AddItem(InDelegate);	};
}
...
{
	if (PrivilegeCheckedDelegates.Find(PrivilegeDelegate) == INDEX_NONE) { PrivilegeCheckedDelegates.AddItem(PrivilegeDelegate);	};
}
...
{
    if (PrivilegeCheckedForUsersDelegates.Find(PrivilegeDelegate) == INDEX_NONE) { PrivilegeCheckedForUsersDelegates.AddItem(PrivilegeDelegate);	};
}
...
{
	if (LoginChangeDelegates.Find(LoginDelegate) == INDEX_NONE) { LoginChangeDelegates.AddItem(LoginDelegate);	};
}
...
{
	if (LoginCancelledDelegates.Find(CancelledDelegate) == INDEX_NONE) { LoginCancelledDelegates.AddItem(CancelledDelegate);	};
}
...
{
	if (CurrentUserChangedDelegates.Find(CurrentUserDelegate) == INDEX_NONE) { CurrentUserChangedDelegates.AddItem(CurrentUserDelegate);	};
}
...
function AddLoginStatusChangeDelegate(delegate<OnLoginStatusChange> LoginStatusDelegate,byte LocalUserNum)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].LoginStatusChangeDelegates.Find(LoginStatusDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].LoginStatusChangeDelegates.AddItem(LoginStatusDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddLoginStatusChangeDelegate()");
	}
}
...
function ClearLoginStatusChangeDelegate(delegate<OnLoginStatusChange> LoginStatusDelegate,byte LocalUserNum)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearLoginStatusChangeDelegate()");
	}
}
...
{
	if (ControllerChangeDelegates.Find(ControllerChangeDelegate) == INDEX_NONE) { ControllerChangeDelegates.AddItem(ControllerChangeDelegate);	};
}
...
{
	if (ConnectionStatusChangeDelegates.Find(ConnectionStatusDelegate) == INDEX_NONE) { ConnectionStatusChangeDelegates.AddItem(ConnectionStatusDelegate);	};
}
...
{
	if (FocusChangeDelegates.Find(FocusChangeDelegate) == INDEX_NONE) { FocusChangeDelegates.AddItem(FocusChangeDelegate);	};
}
...
function AddReadAchievementsCompleteDelegate(byte LocalUserNum,delegate<OnReadAchievementsComplete> ReadAchievementsCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadAchievementsComplete()");
	}
}
...
function ClearReadAchievementsCompleteDelegate(byte LocalUserNum,delegate<OnReadAchievementsComplete> ReadAchievementsCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadAchievementsCompleteDelegate()");
	}
}
...
{
	if (ShowHelpDelegates.Find(HelpDelegate) == INDEX_NONE) { ShowHelpDelegates.AddItem(HelpDelegate);	};
}
...
{
	if (TokenAndSignatureDelegates.Find(tsrDelegate) == INDEX_NONE) { TokenAndSignatureDelegates.AddItem(tsrDelegate);	};
}
...
{
    if (ShowGamerCardCompleteDelegates.Find(GamerCardDelegate) == INDEX_NONE) { ShowGamerCardCompleteDelegates.AddItem(GamerCardDelegate);	};
}
...
{
	if (ReadOnlineProfileDelegates.Find(ReadOnlineProfileDelegate) == INDEX_NONE) { ReadOnlineProfileDelegates.AddItem(ReadOnlineProfileDelegate);	};
}
...
{
	if (RegisterLocalTalkerCompleteDelegates.Find(RegisterLocalTalkerCompleteDelegate) == INDEX_NONE) { RegisterLocalTalkerCompleteDelegates.AddItem(RegisterLocalTalkerCompleteDelegate);	};
}
...
{
	if (UnregisterLocalTalkerCompleteDelegates.Find(UnregisterLocalTalkerCompleteDelegate) == INDEX_NONE) { UnregisterLocalTalkerCompleteDelegates.AddItem(UnregisterLocalTalkerCompleteDelegate);	};
}
...
{
	if (UnregisterRemoteTalkerCompleteDelegates.Find(UnregisterRemoteTalkerCompleteDelegate) == INDEX_NONE) { UnregisterRemoteTalkerCompleteDelegates.AddItem(UnregisterRemoteTalkerCompleteDelegate);	};
}
...
{
	if (RemoteTalkerStatusChangeDelegates.Find(RemoteTalkerStatusChange) == INDEX_NONE) { RemoteTalkerStatusChangeDelegates.AddItem(RemoteTalkerStatusChange);	};
}
...
function AddOnTrialModeUpdateDelegate( delegate<OnTrialModeUpdate> InDelegate ) { if (TrialModeUpdateDelegates.Find(InDelegate) == INDEX_NONE) { TrialModeUpdateDelegates.AddItem(InDelegate);	}; }
