...
function AddReadAvailableProductsCompleteDelegate(byte LocalUserNum, delegate<OnReadAvailableProductsComplete> ReadAvailableProductsCompleteDelegate)
{
	...
	{
		if (PerUserDelegates[LocalUserNum].AvailableProductReadDelegates.Find(ReadAvailableProductsCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].AvailableProductReadDelegates.AddItem(ReadAvailableProductsCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadAvailableProductsCompleteDelegate()");
	}
}
...
function ClearReadAvailableProductsCompleteDelegate(byte LocalUserNum, delegate<OnReadAvailableProductsComplete> ReadAvailableProductsCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadAvailableProductsCompleteDelegate()");
	}
}
...
function AddReadAdditionalProductDetailsCompleteDelegate(byte LocalUserNum, delegate<OnReadAdditionalProductDetailsComplete> ReadAdditionalProductDetailsCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].AdditionalProductDetailsReadDelegates.Find(ReadAdditionalProductDetailsCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].AdditionalProductDetailsReadDelegates.AddItem(ReadAdditionalProductDetailsCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadAdditionalProductDetailsCompleteDelegate()");
	}
}
...
function ClearReadAdditionalProductDetailsCompleteDelegate(byte LocalUserNum, delegate<OnReadAdditionalProductDetailsComplete> ReadAdditionalProductDetailCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadAdditionalProductDetailsCompleteDelegate()");
	}
}
...
function AddReadInventoryItemsCompleteDelegate(byte LocalUserNum, delegate<OnReadInventoryItemsComplete> ReadInventoryItemsCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].InventoryItemReadDelegates.Find(ReadInventoryItemsCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].InventoryItemReadDelegates.AddItem(ReadInventoryItemsCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadInventoryItemsCompleteDelegate()");
	}
}
...
function ClearReadInventoryItemsCompleteDelegate(byte LocalUserNum, delegate<OnReadInventoryItemsComplete> ReadInventoryItemsCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadInventoryItemsCompleteDelegate()");
	}
}
...
function AddConsumeInventoryItemCompleteDelegate(byte LocalUserNum, delegate<OnConsumeInventoryItemComplete> ConsumeInventoryItemCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].ConsumeInventoryItemDelegates.Find(ConsumeInventoryItemCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].ConsumeInventoryItemDelegates.AddItem(ConsumeInventoryItemCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddConsumeInventoryItemCompleteDelegate()");
	}
}
...
function ClearConsumeInventoryItemCompleteDelegate(byte LocalUserNum, delegate<OnConsumeInventoryItemComplete> ConsumeInventoryItemCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearConsumeInventoryItemCompleteDelegate()");
	}
}
...
{
    if (OnMarketplaceItemPurchasedDelegates.Find(InDelegate) == INDEX_NONE) { OnMarketplaceItemPurchasedDelegates.AddItem(InDelegate);	};
}
