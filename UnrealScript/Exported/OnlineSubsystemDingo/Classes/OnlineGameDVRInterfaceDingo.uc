...
function AddReadRecordedClipsCompleteDelegate(byte LocalUserNum, delegate<OnReadRecordedClipsComplete> ReadRecordedClipsCompleteDelegate)
{
	...
	{
		if (PerUserDelegates[LocalUserNum].ReadRecordedClipsDelegates.Find(ReadRecordedClipsCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].ReadRecordedClipsDelegates.AddItem(ReadRecordedClipsCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadRecordedClipsCompleteDelegate()");
	}
}
...
function ClearReadRecordedClipsCompleteDelegate(byte LocalUserNum, delegate<OnReadRecordedClipsComplete> ReadRecordedClipsCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadRecordedClipsCompleteDelegate()");
	}
}
...
function AddRecordEventCompleteDelegate(byte LocalUserNum, delegate<OnRecordEventComplete> RecordEventCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].RecordEventCompleteDelegates.Find(RecordEventCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].RecordEventCompleteDelegates.AddItem(RecordEventCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddRecordEventCompleteDelegate()");
	}
}
...
function ClearRecordEventCompleteDelegate(byte LocalUserNum, delegate<OnRecordEventComplete> RecordEventCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearRecordEventCompleteDelegate()");
	}
}
...
defaultproperties
{
   Name="Default__OnlineGameDVRInterfaceDingo"
