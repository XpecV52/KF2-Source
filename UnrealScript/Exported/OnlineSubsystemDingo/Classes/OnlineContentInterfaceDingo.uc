...
function AddContentChangeDelegate(delegate<OnContentChange> ContentDelegate, optional byte LocalUserNum = 255)
{
	if (LocalUserNum == 255)
	{
		if (ContentChangeDelegates.Find(ContentDelegate) == INDEX_NONE) { ContentChangeDelegates.AddItem(ContentDelegate);	};
	}
	else
	{
		...
		{
			if (PerUserDelegates[LocalUserNum].ContentChangeDelegates.Find(ContentDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].ContentChangeDelegates.AddItem(ContentDelegate);	};
		}
		else
		{
			WarnInternal("Invalid index ("$LocalUserNum$") passed to AddContentChangeDelegate()");
		}
	}
}
...
function ClearContentChangeDelegate(delegate<OnContentChange> ContentDelegate, optional byte LocalUserNum = 255)
{
	...
	else
	{
		...
		{
			WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearContentChangeDelegate()");
		}
	}
}
...
function AddReadDownloadableContentComplete(byte LocalUserNum, delegate<OnReadDownloadableContentComplete> ReadDLCCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].ReadDLCDelegates.Find(ReadDLCCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].ReadDLCDelegates.AddItem(ReadDLCCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadDownloadableContentComplete()");
	}
}
...
function ClearReadDownloadableContentComplete(byte LocalUserNum, delegate<OnReadDownloadableContentComplete> ReadDLCCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadDownloadableContentComplete()");
	}
}
...
function AddReadCrossTitleDownloadableContentComplete(byte LocalUserNum, delegate<OnReadCrossTitleDownloadableContentComplete> ReadCrossTitleDLCCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].ReadCrossTitleDLCDelegates.Find(ReadCrossTitleDLCCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].ReadCrossTitleDLCDelegates.AddItem(ReadCrossTitleDLCCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadCrossTitleContentCompleteDelegate()");
	}
}
...
function ClearReadCrossTitleDownloadableContentComplete(byte LocalUserNum, delegate<OnReadCrossTitleDownloadableContentComplete> ReadCrossTitleDLCCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadCrossTitleContentCompleteDelegate()");
	}
}
...
function AddReadSaveGameDataComplete(byte LocalUserNum,delegate<OnReadSaveGameDataComplete> ReadSaveGameDataCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].ReadSaveGameDelegates.Find(ReadSaveGameDataCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].ReadSaveGameDelegates.AddItem(ReadSaveGameDataCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddReadSaveGameDataComplete()");
	}
}
...
function ClearReadSaveGameDataComplete(byte LocalUserNum,delegate<OnReadSaveGameDataComplete> ReadSaveGameDataCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearReadSaveGameDataComplete()");
	}
}
...
function AddWriteSaveGameDataComplete(byte LocalUserNum,delegate<OnWriteSaveGameDataComplete> WriteSaveGameDataCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].WriteSaveGameDelegates.Find(WriteSaveGameDataCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].WriteSaveGameDelegates.AddItem(WriteSaveGameDataCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddWriteSaveGameDataComplete()");
	}
}
...
function ClearWriteSaveGameDataComplete(byte LocalUserNum,delegate<OnWriteSaveGameDataComplete> WriteSaveGameDataCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearWriteSaveGameDataComplete()");
	}
}
...
function AddDeleteSaveGameDataCompleteDelegate(byte LocalUserNum,delegate<OnDeleteSaveGameDataComplete> DeleteSaveGameDataCompleteDelegate)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < ArrayCount(PerUserDelegates))
	{
		if (PerUserDelegates[LocalUserNum].DeleteSaveGameDelegates.Find(DeleteSaveGameDataCompleteDelegate) == INDEX_NONE) { PerUserDelegates[LocalUserNum].DeleteSaveGameDelegates.AddItem(DeleteSaveGameDataCompleteDelegate);	};
	}
	else
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to AddDeleteSaveGameDataComplete()");
	}
}
...
function ClearDeleteSaveGameDataCompleteDelegate(byte LocalUserNum,delegate<OnDeleteSaveGameDataComplete> DeleteSaveGameDataCompleteDelegate)
{
	...
	{
		WarnInternal("Invalid index ("$LocalUserNum$") passed to ClearDeleteSaveGameDataComplete()");
	}
}
...
defaultproperties
{
   Name="Default__OnlineContentInterfaceDingo"
