...
var transient bool bAxisResetLeft;
var transient bool bAxisResetRight;
var name PerkLevelupSound;
var private const float StickInputThreshold;
var private const float StickResetThreshold;
...
function OnOpen()
{
    ...
    {
        if(KFGRI.PerksAvailableData.bBerserkerAvailable)
        {
            LastPerkIndex = 0;            
        }
        else
        {
            if(KFGRI.PerksAvailableData.bCommandoAvailable)
            {
                LastPerkIndex = 1;                
            }
            else
            {
                if(KFGRI.PerksAvailableData.bSupportAvailable)
                {
                    LastPerkIndex = 2;                    
                }
                else
                {
                    if(KFGRI.PerksAvailableData.bFieldMedicAvailable)
                    {
                        LastPerkIndex = 3;                        
                    }
                    else
                    {
                        if(KFGRI.PerksAvailableData.bDemolitionistAvailable)
                        {
                            LastPerkIndex = 4;                            
                        }
                        else
                        {
                            if(KFGRI.PerksAvailableData.bFirebugAvailable)
                            {
                                LastPerkIndex = 5;                                
                            }
                            else
                            {
                                if(KFGRI.PerksAvailableData.bGunslingerAvailable)
                                {
                                    LastPerkIndex = 6;                                    
                                }
                                else
                                {
                                    if(KFGRI.PerksAvailableData.bSharpshooterAvailable)
                                    {
                                        LastPerkIndex = 7;                                        
                                    }
                                    else
                                    {
                                        if(KFGRI.PerksAvailableData.bSwatAvailable)
                                        {
                                            LastPerkIndex = 8;                                            
                                        }
                                        else
                                        {
                                            if(KFGRI.PerksAvailableData.bSurvivalistAvailable)
                                            {
                                                LastPerkIndex = 9;                                                
                                            }
                                            else
...
event OnClose()
{
    ...
    if(KFPC != none)
    {
        if((bModifiedPerk || bModifiedSkills) || bModifiedWeaponIndexes)
        {
            ...
            {
                Manager.CachedProfile.SetProfileSettingValueInt(179, KFPC.SurvivalPerkWeapIndex);
                Manager.CachedProfile.SetProfileSettingValueInt(180, KFPC.SurvivalPerkGrenIndex);
            }
            bModifiedPerk = false;
...
function OnGamepadAxisModified(int ControllerId, name Key, float Delta, float DeltaTime, bool bGamepad)
{
    ...
    AbsDelta = Abs(Delta);
    if(KFPC.CurrentPerk.CanChoosePrimaryWeapon() && Key == 'XboxTypeS_LeftX')
    {
        ...
        {
            if(!bAxisResetLeft && AbsDelta < StickResetThreshold)
            {
                bAxisResetLeft = true;
            }
        }        
    ...
    {
        if(KFPC.CurrentPerk.CanChooseGrenade() && Key == 'XboxTypeS_RightX')
        {
            ...
            {
                if(!bAxisResetRight && AbsDelta < StickResetThreshold)
                {
