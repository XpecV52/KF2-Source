...
function Actor FindEnemyFor(KFAIController RequestingAI, optional bool bSkipCurrentEnemy)
{
    ...
    if(P != none)
    {
        if((!RequestingAI.Pawn.IsSameTeam(P) && P.IsAliveAndWell()) && P != RequestingAI.Enemy)
        {
            ...
            {
                if((((DistToPlayer < DistToEnemy) && FRand() < 0.9) || ((KFPawn(P).IsDoingSpecialMove(20) && DistToPlayer < 1200) && RequestingAI.CanSee(P)) && FRand() < 0.85) || (float(RequestingAI.Pawn.Health) < (0.33 * float(RequestingAI.Pawn.default.Health))) && FRand() < 0.65)
                {
                    return P;
                }
            }
